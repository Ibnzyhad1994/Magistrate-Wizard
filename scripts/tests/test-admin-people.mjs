...
const rows = buildAdminPeopleRows({
  profiles: [
    {
      id: "m1",
      full_name: "Local Magistrate",
      email: "magistrate@court.gy",
      role: "magistrate",
      is_active: true,
    },
    {
...
check(
  "admin keeps the later login, not an earlier one",
  admin?.lastLoginAt,
  "2026-09-01T12:00:00Z",
);
check(
  "login can match by email when actor_id is missing",
  magistrate?.lastLoginAt,
...
check(
  "admin acting seating is listed",
  admin?.courts.map((c) => `${c.courtName}:${c.assignmentType}`),
  ["Georgetown:acting"],
);
check("clerk court is tagged as clerk kind", clerk?.courts[0]?.kind, "clerk");
check(
  "latest activity wins over an older sign-in",
  admin?.lastActivityLabel,
  "Magistrate assigned to a court",
);
check(
  "latest activity timestamp is the later event",
...
const unorderedLogin = buildAdminPeopleRows({
  profiles: [
    { id: "a1", full_name: "Admin", email: "admin@court.gy", role: "admin", is_active: true },
  ],
  magistrateAssignments: [],
  clerkAssignments: [],
  loginEvents: [
...
check(
  "last login is the newest timestamp even if events are not pre-sorted",
  unorderedLogin[0]?.lastLoginAt,
  "2026-09-01T12:00:00Z",
);
