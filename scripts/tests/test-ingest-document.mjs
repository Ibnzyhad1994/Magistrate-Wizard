...
const main = async () => {
  // --- classifyIngestSource ---
  check(
    "classify pdf by mime",
    classifyIngestSource({ name: "a.bin", type: "application/pdf" }),
    "pdf",
  );
  check(
    "classify pdf by extension",
    classifyIngestSource({ name: "Judgment.PDF", type: "" }),
    "pdf",
  );
  ...
  check(
    "classify docx by extension not as legacy doc",
    classifyIngestSource({ name: "Ramsingh.docx", type: "" }),
    "docx",
  );
  check(
    "classify legacy doc",
    classifyIngestSource({ name: "old.doc", type: "application/msword" }),
    "doc",
  );
  check("classify image jpeg", classifyIngestSource({ name: "scan.JPG", type: "" }), "image");
  check(
    "classify unsupported",
    classifyIngestSource({ name: "virus.exe", type: "application/x-msdownload" }),
    "unsupported",
  );

  check(
    "infer mime for empty-type md",
    inferStoredMimeType({ name: "notes.md", type: "" }),
    "text/markdown",
  );
  check(
    "infer mime for docx",
  ...
  }
  {
    ...
    checkTrue(
      "short txt still keeps the characters for the curator to edit",
      envelope.text.includes("Hello"),
    );
  }
  {
    ...
    checkTrue(
      "markdown keeps heading source",
      envelope.text.includes("The State v Dhannie Ramsingh"),
    );
  }
  ...
  // --- .doc is honest pending, never fabricated ---
  {
    ...
    checkTrue(
      "legacy doc warning mentions paste or docx",
      /docx|paste/i.test(envelope.warnings.join(" ")),
    );
  ...
  check(
    "preview md by name despite text/plain mime",
    getDocumentPreviewKind("text/plain", "notes.md"),
    "markdown",
  );
  ...
  check(
    "preview legacy doc is unsupported",
    getDocumentPreviewKind("application/msword", "old.doc"),
    "unsupported",
  );
  checkTrue("isLegacyWordDocument .doc", isLegacyWordDocument("application/msword", "old.doc"));
  checkTrue(
    "isLegacyWordDocument does not treat docx as legacy",
    !isLegacyWordDocument("", "Ramsingh.docx"),
  );
  ...
  // --- XSS ---
  {
    ...
    checkTrue(
      "markdown escapes script tags",
      html.includes("&lt;script&gt;") && !html.includes("<script>"),
    );
    checkTrue(
      "markdown does not emit javascript: href",
  ...
  // --- magic-byte upload guard ---
  {
    ...
    try {
      await assertFileContentMatchesKind(
        makeFile("evil.pdf", new Uint8Array([0x4d, 0x5a, 0x90, 0x00]), "application/pdf"),
      );
    } catch {
      peRejected = true;
    }
    ...
    try {
      await assertFileContentMatchesKind(
        makeFile("page.pdf", "<script>alert(1)</script>", "application/pdf"),
      );
    } catch {
  ...
  checkTrue(
    "validator rejects exe",
    !validateFileForUpload(makeFile("virus.exe", new Uint8Array([1]), "application/x-msdownload"))
      .ok,
  );
