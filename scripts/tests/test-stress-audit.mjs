...
const signIn = async (email) => {
  ...
  return {
    ...
    client: clientAs(data.session.access_token),
  };
};
...
check(
  "bucket recorded 60 successful hits for user A",
  hitsA >= 60 || (allBuckets ?? []).some((row) => row.hit_count >= 60),
  true,
);

const { data: leaked, error: leakError } = await userA.client
  .from("rpc_rate_limit_buckets")
  .select("hit_count");
check(
  "authenticated client cannot read rate-limit buckets",
  Boolean(leakError) || (leaked ?? []).length === 0,
  true,
);
...
for (let p = 0; p < 12; p++) {
  ...
  for (let line = 0; line < 40; line++) {
    fat.text(
      `Paragraph ${line + 1} of page ${p + 1}. Police v. Demo Defendant.`,
      72,
      110 + line * 14,
    );
  }
...
check(
  "burn does not mutate source bytes",
  originalCopy.every((b, i) => b === fatBytes[i]),
  true,
);
...
const { ms: parallelBurnMs } = await timed("3 concurrent one-page burns", () =>
  Promise.all([
    ...
    burnRedactedPdf({
      bytes: tinyBytes.slice(),
      boxes: overlapBoxes.slice(5, 10),
      title: "p2.pdf",
    }),
    burnRedactedPdf({
      bytes: tinyBytes.slice(),
      boxes: overlapBoxes.slice(10, 15),
      title: "p3.pdf",
    }),
  ]),
);
