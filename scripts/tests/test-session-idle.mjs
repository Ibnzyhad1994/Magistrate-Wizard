...
check(
  "activity within the hour stays ok",
  evaluateSessionIdle({ lastActivityAt: t0, now: t0 + hour - WARN_BEFORE_MS - 1 }),
  "ok",
);
check(
...
check(
  "lock at 60 minutes idle",
  evaluateSessionIdle({ lastActivityAt: t0, now: t0 + hour }),
  "lock",
);
check(
...
check(
  "rememberUntil not expired at the boundary",
  isRememberExpired(t0 + REMEMBER_MS, t0 + REMEMBER_MS),
  false,
);
check(
  "rememberUntil expired after 14 days",
  isRememberExpired(t0 + REMEMBER_MS, t0 + REMEMBER_MS + 1),
  true,
);

check(
  "login from restores path+search+hash",
  pathFromLoginRedirect(
    {
      pathname: "/docket/abc",
      search: "?court=1",
  ...
  "/docket/abc?court=1#notes",
);
check(
  "protocol-relative from is rejected",
  pathFromLoginRedirect({ pathname: "//evil.example" }, "/dashboard"),
  "/dashboard",
);
...

{
  ...
  check(
    "legacy localStorage session moves to sessionStorage",
...

{
  ...
  check(
    "Remember me writes localStorage",
...

{
  ...
  check(
    "session-only writes sessionStorage",
    session.getItem(AUTH_STORAGE_KEY),
    '{"access_token":"tab"}',
  );
...

{
  ...
  local.setItem(REMEMBER_PREFERENCE_KEY, JSON.stringify({ rememberMe: true, rememberUntil: t0 }));
