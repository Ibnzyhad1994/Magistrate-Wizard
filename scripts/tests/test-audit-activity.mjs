...
check(
  ...
  summarizeChange(
    "profiles",
    "update",
    { role: "magistrate", email: "a@court.gy", full_name: "A Magistrate" },
    { role: "admin", email: "a@court.gy", full_name: "A Magistrate" },
  ).title,
...
check(
  ...
  summarizeChange(
    "profiles",
    "update",
    { is_active: true, email: "a@court.gy" },
    { is_active: false, email: "a@court.gy" },
  ).title,
  "Account deactivated",
);
...
check("sign-in filter does not query change tables", tablesForFilter("signin"), []);
...
check(
  ...
  [
    actorDisplayName({ full_name: "Sam", email: "s@x" }),
    actorDisplayName(null, "s@x"),
    actorDisplayName(null, null),
  ],
  ["Sam", "s@x", "Unknown"],
);
...
check("docket is an institutional table and has its own filter", tablesForFilter("docket"), [
  "docket_matters",
]);

check(
  "docket create names the case",
  summarizeChange("docket_matters", "insert", null, {
...
check(
  ...
  summarizeChange(
    "docket_matters",
    "delete",
    { case_number: "123/2026", matter_title: "Police v. Doe" },
    null,
  ).title,
...
check(
  ...
  summarizeChange("docket_matters", "insert", null, { case_number: "1", matter_title: "A" })
    .category,
  "docket",
);
