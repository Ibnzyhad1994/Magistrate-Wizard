  ...
  try {
    ...
    if (ndErr) fail(tag, "set next date", ndErr);
      ...
      log(
        ...
        tag,
        "set next date",
        nextDate?.[0]?.status ?? JSON.stringify(nextDate)?.slice(0, 80),
      );
    ...
    if (evErr) fail(tag, "calendar events for matter", evErr);
      ...
      log((events?.length ?? 0) > 0, tag, "calendar events for matter", `${events?.length ?? 0}`);
    ...
    if (binErr) fail(tag, "bin matter", binErr);
      ...
      const { data: binned } = await sb
        ...
        .maybeSingle();
      log(
        !hidden && !!binned?.deleted_at,
        tag,
        "binned matter hidden from live list",
        hidden ? "still on live filter" : "hidden; restore path still readable",
      );
...
const runClerk = async (actor, court) => {
  ...
  try {
    ...
    if (ccErr) fail(tag, "list clerk_courts", ccErr);
      ...
      log(
        (clerkCourts?.length ?? 0) > 0,
        tag,
        "clerk_courts assignment",
        clerkCourts?.[0]?.courts?.name ?? "",
      );
    ...
    log(
      !!assignErr,
      tag,
      "cannot self-assign magistrate_courts",
      assignErr?.message ?? "INSERT succeeded",
    );

    const { data: issueReports, error: irErr } = await sb
      .from("issue_reports")
      .select("id")
      .limit(5);
    if (irErr) log(true, tag, "issue_reports blocked or empty", irErr.message);
      ...
      log(
        (issueReports?.length ?? 0) === 0,
        tag,
        "cannot browse others' issue reports",
...
const runOutsider = async (actor, foreignMatterId, geo1Id) => {
  ...
  try {
    ...
    log(
      !!insErr,
      tag,
      "cannot create matter on foreign court",
      insErr?.message ?? "INSERT succeeded",
    );
...
const runEmpty = async (actor, foreignMatterId) => {
  ...
  try {
    ...
    log(
      (courts?.length ?? 0) > 0,
      tag,
      "has court but empty docket",
      courts?.[0]?.courts?.name ?? "",
    );
...
const main = async () => {
  ...
  try {
    const emptyCourt = await pickOpenRegularCourt([
      geo1.id,
      acquero.id,
      noviceCourt.id,
      coveringCourt.id,
