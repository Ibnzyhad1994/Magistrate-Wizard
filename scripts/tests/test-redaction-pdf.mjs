...
check(
  "undo last box",
  undoRedaction([{ pageNumber: 1, x: 0, y: 0, width: 1, height: 1 }]).length,
  0,
);
...
check(
  "removing the first box",
  removeRedactionBoxAt(many, 0).map((x) => x.x),
  [0.2, 0.3, 0.4],
);
check(
  "removing the last box",
  removeRedactionBoxAt(many, 3).map((x) => x.x),
  [0.1, 0.2, 0.3],
);
...
check(
  "the DISPLAY copy is not what is matched",
  isRateLimitedError({ message: "Too many requests. Try again in a minute." }),
  false,
);
...
check(
  ...
  originalCopy.length === fixtureBytes.length &&
    originalCopy.every((b, i) => b === fixtureBytes[i]),
  true,
);
