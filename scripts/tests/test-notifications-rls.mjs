...
try {
  ...
  // --- 0156: issue-report status changes reach the reporter ----------------
  // `other` is the seeded admin; `owner` files the report. Two decisions
  // the same day must both arrive (no 20 h type+link dedupe on this path).
  const { data: report, error: reportErr } = await owner.client
    .from("issue_reports")
    .insert({
      reporter_id: owner.id,
      type: "bug",
      title: "RLS fixture: notify reporter",
      description: "fixture",
    })
    .select("id")
    .single();
  ...
  try {
    await other.client.from("issue_reports").update({ status: "in_progress" }).eq("id", report.id);
    await other.client.from("issue_reports").update({ status: "resolved" }).eq("id", report.id);
    const { data: reporterNotices } = await owner.client
      .from("notifications")
      .select("id, type, title, body")
      .eq("type", "issue_report_decided")
      .order("created_at", { ascending: false })
      .limit(5);
    const mine = (reporterNotices ?? []).filter((n) => n.body === "RLS fixture: notify reporter");
    created.push(...mine.map((n) => n.id));
    check("reporter is told each time the status changes", mine.length, 2);
    check(
      "the notice says what the new status is",
      mine.some((n) => /resolved/.test(n.title)) && mine.some((n) => /in progress/.test(n.title)),
      true,
    );
    await other.client
      .from("issue_reports")
      .update({ admin_notes: "notes only" })
      .eq("id", report.id);
    const { count: afterNotes } = await owner.client
      .from("notifications")
      .select("id", { count: "exact", head: true })
      .eq("type", "issue_report_decided")
      .eq("body", "RLS fixture: notify reporter");
    check("a change that is not a status change sends nothing", afterNotes, 2);
  } finally {
    await admin.from("issue_reports").delete().eq("id", report.id);
  }
} finally {
