...
const main = async () => {
  console.log(
    `Slow-system OCR simulation — SLOW_PAGE_MS=${SLOW_PAGE_MS}, MAX_OCR_PAGES=${MAX_OCR_PAGES}`,
  );
  ...
  // ------------------------------------------------------------------
  {
    const file = makeDegradedScanPdf({
      dpi: 150,
      jpegQuality: 40,
      noiseRatio: 2,
      name: "slow-noisy.pdf",
  ...
  console.log(
    `Wall ${payload.elapsedMs}ms  mem ${memStart.heapUsed}→${payload.memEnd.heapUsed} MB heap`,
  );
