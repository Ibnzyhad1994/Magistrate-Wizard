...
const main = async () => {
  ...
  // Heavier JPEG compression still has to recover the citation and parties.
  {
    ...
    check(
      "compressed scan contains case name",
      containsNormalized(envelope.text, "DHANNIE RAMSINGH"),
      true,
    );
    check(
      "compressed scan contains citation",
      containsNormalized(envelope.text, "20 WIR 138"),
      true,
    );
    const cer = characterErrorRate(envelope.text, SCAN_GROUND_TRUTH);
