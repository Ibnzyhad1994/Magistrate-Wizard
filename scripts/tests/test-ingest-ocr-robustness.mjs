  ...
  record({
    ...
    detail: !pass
      ? [
          ...
          expectMethod && envelope.method !== expectMethod
            ? `method ${envelope.method} != ${expectMethod}`
            : null,
          expectOcr != null && envelope.ocrUsed !== expectOcr
            ? `ocrUsed ${envelope.ocrUsed} != ${expectOcr}`
            : null,
          envelope.warnings?.[0],
        ]
          .filter(Boolean)
...
const main = async () => {
  ...
  }
  {
    ...
    const hasAccents =
      envelope.text.includes("naïve") &&
      envelope.text.includes("café") &&
      envelope.text.includes("résumé");
    record({
      family: "born-digital",
  ...
  {
    const file = makeMixedEncodingPdf(
      GOLDEN_LINES.slice(0, 4),
      GOLDEN_LINES.slice(4),
      "mixed-encoding.pdf",
    );
    const envelope = await ingestDocument(file);
    assertExactish({
  ...
  {
    const envelope = await ingestDocument(
      makeConcatenatedTextPdf(GOLDEN_PROSE, "concatenated.pdf"),
    );
    assertExactish({
      family: "pdf-encoding",
  ...
  }
  {
    ...
    record({
      ...
      pass:
        !usable(envelope) &&
        envelope.text === "" &&
        !MUST.some((p) => containsNormalized(envelope.text, p)),
      status: envelope.status,
      method: envelope.method,
      ocrUsed: envelope.ocrUsed,
      unreadableReason: envelope.unreadableReason,
  ...
  {
    const envelope = await ingestDocument(
      makeFile("virus.exe", new Uint8Array([0x4d, 0x5a]), "application/x-msdownload"),
    );
    record({
      family: "honesty",
  ...
  const outPath = join(
    ...
    "ingest-ocr-robustness-results.json",
  );
  const summary = {
