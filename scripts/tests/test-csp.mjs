...
check(
  "frame-src allows the supabase origin (PDF iframe)",
  frameSrc,
  `frame-src 'self' blob: ${production}`,
);
check(
  "img-src allows the supabase origin (image preview)",
  csp.includes(`img-src 'self' blob: data: ${production}`),
  true,
);
check(
  "connect-src still allows the supabase origin",
...
// Fonts are self-hosted through @fontsource (src/main.tsx). The Google
// Fonts origins were the only third-party style/font sources; if either
// creeps back in, a courthouse with no outbound internet gets a
// render-blocking stylesheet request and the native shells a CSP hole.
check("style-src carries no Google Fonts origin", csp.includes("fonts.googleapis.com"), false);
check("font-src carries no Google Fonts origin", csp.includes("fonts.gstatic.com"), false);
check("font-src is self plus data: only", csp.includes("font-src 'self' data:"), true);
check(
  "style-src is self plus 'unsafe-inline' only",
  csp.split("; ").includes("style-src 'self' 'unsafe-inline'"),
  true,
);
check(
  "index.html no longer links Google Fonts",
  readFileSync("index.html", "utf8").includes("fonts.googleapis.com"),
  false,
);

const localCsp = buildCsp(local);
check(
  "local frame-src uses the local origin, not production",
  localCsp.includes(`frame-src 'self' blob: ${local}`),
  true,
);
check(
...
check(
  "the CSP carries a hash matching the real inline script",
  hashedCsp.includes(expectedHash),
  true,
);
...
check(
  "script-src does not gain 'unsafe-inline' — the hash is the whole point",
  scriptSrc.includes("unsafe-inline"),
  false,
);
check(
  "style-src still has 'unsafe-inline' (Radix/Tailwind inline styles)",
  hashedCsp.includes("style-src 'self' 'unsafe-inline'"),
  true,
);
check("'self' survives alongside the hash", hashedCsp.includes("script-src 'self' 'sha256-"), true);
check(
  "wasm-unsafe-eval is preserved (pdf.js/tesseract need it)",
  hashedCsp.includes("'wasm-unsafe-eval'"),
  true,
);
