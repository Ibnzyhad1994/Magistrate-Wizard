...
check(
  "the three sitting statuses are the ones the CHECK constraint allows",
  [...CALLOVER_STATUSES],
...
check(
  "every outcome value is unique",
  CALLOVER_OUTCOME_VALUES.length,
  new Set(CALLOVER_OUTCOME_VALUES).size,
);
...
check(
  "Adjourned never suggests completing the matter",
  outcomeSuggestsCompletion("Adjourned"),
  false,
);

check(
  "an outcome that suggests completion never also expects a return date",
...
check(
  "free-text outcome is accepted and simply carries no hints",
  calloverOutcomeMeta("Referred to the DPP"),
  null,
);
check("free-text outcome expects no date", outcomeExpectsNextDate("Referred to the DPP"), false);
check(
  "free-text outcome suggests no completion",
  outcomeSuggestsCompletion("Referred to the DPP"),
  false,
);
...
check(
  "a tied sort_order falls back to created_at, so rows never swap between renders",
  sortCalloverItems([
...
check(
  "sorting does not move called rows — the magistrate keeps their place",
  sortCalloverItems([
...
check("an empty list reports zero rather than NaN", calloverProgress([]), {
  total: 0,
  called: 0,
  remaining: 0,
  percent: 0,
...
check("a fully-called list reports 100", calloverProgress([item({ called_at: "x" })]), {
  total: 1,
  called: 1,
  remaining: 0,
  percent: 100,
...
check(
  "flags a called Adjourned row left without a return date",
  itemsMissingExpectedNextDate([
...
check(
  "does not flag an Adjourned row that HAS a return date",
  itemsMissingExpectedNextDate([
...
check(
  "does not flag an uncalled row, however it is left",
  itemsMissingExpectedNextDate([item({ called_at: null, outcome: "Adjourned", next_date: null })]),
  [],
);

check(
  "does not flag Struck out, which legitimately has no return date",
  itemsMissingExpectedNextDate([item({ called_at: "x", outcome: "Struck out", next_date: null })]),
  [],
);
...
check(
  "a malformed date still yields a usable title",
  defaultCalloverTitle("not-a-date"),
  "Callover",
);
check(
...
check(
  "an unseeded matter still starts at Arraignment",
  currentStage({
...
check(
  "a fully-disposed matter seeds to Appeal",
  currentStage({
    arraignment_status: "done",
    custody_status: "on_bail",
    disclosure_status: "full",
