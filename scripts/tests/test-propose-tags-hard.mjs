...
const expectEmpty = (label, tags) => record(label, tags.length === 0, `got=[${tags.join(", ")}]`);
...
// ---------------------------------------------------------------------------
{
  const traps = [
    [
      "research / researcher / researched",
      "Counsel cited academic research by a researcher who researched the point.",
      "Search",
    ],
    [
      "service industry prose",
      "The customer service industry provides a valuable service daily.",
      "Service",
    ],
    [
      "knowledge everyday",
      "To my knowledge the meeting was cancelled for lack of knowledge sharing.",
      "Knowledge",
    ],
    [
      "debt metaphorical",
      "We are in debt to prior courts for their reasoning on unrelated debt instruments in literature.",
      "Debt",
    ],
    [
      "custody of documents figurative once",
      "The registrar retained custody of the file overnight.",
      "Custody",
    ],
    ["quantity of time", "A large quantity of time was wasted on adjournments.", "Quantity"],
    [
      "possession of skill once",
      "Counsel showed possession of considerable skill in cross-examination.",
      "Possession",
    ],
    ...
    [
      "maintenance of order (not family)",
      "Maintenance of order in the courtroom is the duty of the police.",
      "Maintenance",
    ],
    [
      "maintenance twice everyday",
      "Maintenance of order and maintenance of records are clerical duties.",
      "Maintenance",
    ],
    [
      "service×2 still below bare floor",
      "Customer service failed; the service desk closed early.",
      "Service",
    ],
    [
      "knowledge×2 still below bare floor",
      "To my knowledge nobody attended. Knowledge sharing was poor.",
      "Knowledge",
    ],
    [
      "debt×2 still below bare floor",
      "In debt to history, and a debt of gratitude remains.",
      "Debt",
    ],
  ];
...
// ---------------------------------------------------------------------------
{
  ...
  expectIncludes(
    ...
    proposeTags(
      "Possession was denied. Later possession was argued. The court found possession proved.",
    ),
    "Possession",
  );
  expectExcludes(
  ...
  expectIncludes(
    "floor — Search thrice",
    proposeTags(
      "A search of the vehicle followed a search of the bag and a further search of the home.",
    ),
    "Search",
  );
  expectIncludes(
...
// ---------------------------------------------------------------------------
{
  ...
  const visual = proposeTags("The case turned on visual identification by a single dock witness.");
...
// ---------------------------------------------------------------------------
{
  ...
  record(
    "Ramsingh — proposal count sensible (1–10)",
    tags.length >= 1 && tags.length <= 10,
    `n=${tags.length} [${tags.join(", ")}]`,
  );
}
...
// ---------------------------------------------------------------------------
{
  const pad = "The court adjourned without comment on unrelated administrative matters. ".repeat(
    400,
  );
  const needle =
...
// ---------------------------------------------------------------------------
{
  ...
  expectIncludes(
    ...
    proposeTags(
      "This was not hearsay. Later the court admitted hearsay evidence on another count.",
    ),
    "Hearsay",
  );
  expectIncludes(
...
// ---------------------------------------------------------------------------
{
  ...
  record(
    "section — HELD hit proposes Hearsay",
    !!inHeld,
    inHeld ? `score=${inHeld.score}` : "missing",
  );
