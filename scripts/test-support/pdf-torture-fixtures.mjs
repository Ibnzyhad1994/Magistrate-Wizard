...
import {
  makeValidScannedJpegPdf,
  renderLegalScanJpeg,
  SCAN_GROUND_TRUTH,
} from "./scanned-pdf-fixtures.mjs";
...
export const makeAsciiHexStreamPdf = (lines, name = "asciihex.pdf") => {
  ...
  return toFile(
    assemblePdf([{ dict: `/Length ${hex.length} /Filter /ASCIIHexDecode`, bytes: hex }]),
    name,
  );
};
...
export const makeAlignedSearchableScanPdf = (opts = {}) => {
  ...
  return new File([Buffer.concat(parts)], opts.name ?? "searchable-aligned.pdf", {
...
export const makeMismatchedOverlayPdf = (opts = {}) => {
  ...
  parts.push(ascii(xrefLines.join("")));
  parts.push(ascii(`trailer\n<< /Size 8 /Root 1 0 R >>\nstartxref\n${xrefStart}\n%%EOF`));
  return new File([Buffer.concat(parts)], opts.name ?? "searchable-mismatch.pdf", {
    type: "application/pdf",
  });
};
...
export const makeMultiPageScannedPdf = (pageCount, opts = {}) => {
  ...
  return new File([Buffer.concat(parts)], opts.name ?? `scan-${pageCount}p.pdf`, {
    type: "application/pdf",
  });
};
