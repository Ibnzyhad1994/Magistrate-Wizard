# Magistrate Wizard Authoritative Architecture Specification (Final, Pre-Implementation)
...
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docket_matters`                                      | Court-anchored — visible/editable to any magistrate with a **current** `magistrate_courts` assignment to that matter's court                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **Yes** — per-matter view/edit share with a registered user, for exceptional consultation only (not ordinary succession). _Design fully resolved and prepared for review as `0037_shares.sql` (not yet applied) — see §14: grant/revoke authority = any current lawful Docket-access holder (court OR retained), plus the recipient may always relinquish their own share; no resharing; soft-revocation only (`revoked_at`); at most one active share per recipient per matter; share grants access to the Docket Matter row only, not its events/parties/tags/associations in this migration._ | **No — never**                                                                                                                                                                                                                                                                                                               | **No.** Admin has zero special access to another court's or magistrate's Docket content, including no visibility into `shares` rows they neither granted nor received. |
| `docket_matter_assignments` (retained/part-heard)     | The named magistrate only, for that specific matter, while `ended_at IS NULL`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | N/A — this _is_ the access grant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | No                                                                                                                                                                                                                                                                                                                           | **No.**                                                                                                                                                                |
...
| `case_law` (canonical, `owner_id IS NULL`)            | Visible to all authenticated users                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | N/A (already universal)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | N/A (already universal)                                                                                                                                                                                                                                                                                                      | **Yes, for write only** — expressly administrative resource. _(Resolved/implemented when `0035_case_law_personal_research` was prepared — not yet applied; see §5.)_   |
| `case_law` (personal, `owner_id` set)                 | Private to owner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **Yes** — view or edit, registered users only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Yes** — owner-controlled `is_discoverable` flag                                                                                                                                                                                                                                                                            | **No — never, even for canonical-curation admins.** _(Resolved/implemented when `0035_case_law_personal_research` was prepared — not yet applied; see §5.)_            |
| `case_law_annotations`                                | Strictly private to the annotation's own `owner_id` — the parent Case Law's own owner has no visibility into it either                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **No, ever**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | No                                                                                                                                                                                                                                                                                                                           | **No, ever.** _(Resolved/prepared for review when `0036_case_law_annotations` was written — not yet applied; see §5.)_                                                 |
...
`magistrate_courts` is an **authority-originating** table, not a preference or a continuation of already-legitimate access. A row in this table is the sole input to `can_access_court()`, which is in turn one of the three paths feeding every Docket read/write predicate in §14. Creating a `magistrate_courts` row therefore _grants_ Court-wide Docket authority from nothing — it does not confirm or preserve access the caller already holds by some other means.

This is the decisive difference from `docket_matter_assignments` (retained/part-heard, §4 above, self-service since `0022`): a retained assignment can only be created by a magistrate who, at the moment of creation, _already_ holds ordinary Court access to that specific matter via `can_access_court()` — it narrows and preserves a pre-existing, already-legitimate grant down to one matter as the magistrate's broader Court access is about to end. It can never be used to originate access the caller doesn't already have. `magistrate_courts`, by contrast, has no such precondition available to check — there is nothing upstream of it to require, because it _is_ the upstream grant. Self-service at this layer would mean any authenticated user could grant themselves Court-wide authority over any active Court with no precondition at all, which is a fundamentally different (and unacceptable) trust decision from self-service at the retained-assignment layer.
...
**Magistrate offboarding (retirement, resignation, promotion, or otherwise ceasing to use Magistrate Wizard) — resolved when migration 0022 was prepared:**
...
- **Normal judicial departure** (retirement, resignation, promotion/appointment to another judicial office, transfer out of the Magistracy, or other ordinary cessation of Magistrate Wizard use) should NEVER delete the person's `profiles` row. The preferred future model is: (1) disable/deactivate their ability to authenticate/use Magistrate Wizard; (2) end their current `magistrate_courts` assignments; (3) end their current `docket_matter_assignments`; (4) preserve their `profiles` row as historical judicial identity; (5) preserve all historical Court and Docket assignment rows. Ceasing to use Magistrate Wizard must never, by itself, require deleting a magistrate's historical identity — historical records should remain able to identify which magistrate previously held a given Court or retained/part-heard assignment, by name, not just by an orphaned reference.
...
- **Institutional Docket tags** (`docket_matter_tags`, built in 0026): belong to the Docket Matter itself — operational/workflow labels (illustrative only, not seeded or hard-coded: e.g. _part-heard_, _urgent_, _awaiting report_, _warrant outstanding_). Follow the Court Docket; visible to every magistrate with lawful access to that matter (current Court assignment or retained assignment); not personal to whoever created them; will eventually also follow explicit parent-matter `shares` once that path exists.
- **Personal magistrate labels** (e.g. _review Friday_, _read before hearing_, _discuss with clerk_): private to the individual magistrate, NOT part of 0026, not designed here. A future, separate, user-owned/private mechanism. Explicitly must **not** automatically transfer to a successor magistrate merely because the successor inherits ordinary Court-wide Docket access — a private label is not Docket metadata.
...
judgments (
  ...
  court_name text,               -- optional free text; deliberately NOT
                                   ...
                                   -- the active Magistrate Wizard Court
                                   -- reference structure
...
_(unchanged from Revision 2 — polymorphic parent, `author_id = auth.uid()` only, no admin clause, no cascading from parent access — explicitly reaffirmed as correct and untouched by the Docket pivot in §12/§14 below)_

---

## 9. Final Documents/Storage Architecture

_(unchanged in mechanism — access dispatches to the parent entity's access rule, which for `docket_matter`-typed documents now means the three-path Docket predicate in §14 instead of ownership)_
...
- ~~Docket↔Judgment association authority, join-row visibility, and duplicate prevention~~ → **Resolved when `0029_docket_matter_judgments` was prepared — see §6/§14. The association confers no access in either direction (unchanged principle, now field-level resolved). SELECT requires independent lawful access to BOTH the Docket Matter and the Judgment (`DocketAccess AND JudgmentAccess`, never `OR`) — this closes a gap the earlier resolution left open, where the mere existence/visibility of a link row could otherwise have leaked information about an inaccessible record. INSERT and DELETE both require lawful Docket access AND Judgment _ownership_ specifically — `is_discoverable` alone is never sufficient to create or remove a link, and DELETE uses the identical authority as INSERT rather than deferring to whoever originally created the row (`created_by` is provenance only, never access control). `unique(docket_matter_id, judgment_id)` prevents duplicates while preserving genuine many-to-many cardinality. No UPDATE policy (the association has no editable fields). No admin bypass. The join row carries no descriptive metadata — only its two foreign keys and creation provenance. The future `shares` path is recorded in principle (both sides must independently satisfy their own eventual share-based access before an association becomes visible) but not implemented; whether share-based _edit_ access should extend to link creation/removal is left an explicit, undecided future question.**
...
## 18. Turn 3 — Consolidated Product/UX/Search/Date-Time Pass

**No new migration in this turn — 0052 remains the latest applied migration.** Every change below is frontend-only (React/TypeScript, `src/`), reusing existing, already-applied schema/RLS exactly as found. Where a genuine schema change would have been the "more complete" answer, that is disclosed explicitly as DEFERRED rather than attempted as a rushed, under-reviewed migration late in an already large session — consistent with this project's own gated-migration discipline (inspect → pretest → apply → verify, never skipped).

**A/B — Date/time root cause and fix (`src/lib/utils.ts`).** Root cause: `formatDate()` fed a bare Postgres `date` string (e.g. `"2026-08-20"`, no time/zone) straight into `new Date(...)`, which the ECMAScript spec parses as **UTC midnight**; `Intl.DateTimeFormat` then rendered that instant in the browser's LOCAL zone, silently rolling the displayed calendar date back one day for any negative-UTC-offset zone (Guyana, UTC-4 — UTC midnight Aug 20 is 8pm local Aug 19). Fix: a new `parseDateOnly()` splits the string into numeric Y/M/D and calls `new Date(y, m-1, d)`, which JS always interprets as LOCAL midnight — the calendar day can no longer move. `formatDate()` now detects a bare `YYYY-MM-DD` string via regex and routes it through `parseDateOnly()`; anything else (a full ISO timestamp, or a `Date`) is untouched, so `created_at`/`updated_at`/`finalized_at`/`started_at`/`ended_at`/`revoked_at` — genuine `timestamptz` instants — keep their existing, correct behavior. A second, related bug was found and fixed in `useUpcomingAppearances()` (`src/hooks/use-dashboard.ts`): `new Date().toISOString().slice(0,10)` computed "today" in UTC, not local time, which is wrong as a `.gte("scheduled_date", today)` boundary for the same reason; replaced with a new `getLocalDateOnly()` helper built from local `Date` accessors. Verified with a Node script run under `TZ=America/Guyana` (UTC-4), `TZ=Pacific/Kiritimati` (UTC+14), `TZ=UTC`, and `TZ=Asia/Kolkata` (UTC+5:30): the old code reproduced the exact reported bug (20 Aug → "Aug 19, 2026") in Guyana only; the new code reads "20 Aug 2026" in all four zones. No `+1`/`-1` day arithmetic anywhere — this was a parsing-boundary fix, per the explicit instruction not to paper over it with day arithmetic. Presentation standard (Part B): `formatDate`'s `Intl.DateTimeFormat` locale changed from `"en-US"` to `"en-GB"` (day-before-month, e.g. "20 Aug 2026", Commonwealth-style), applied everywhere `formatDate`/`formatDateTime` is already called — no per-page changes needed since this is the single shared formatter. A new `formatTimeOnly()` renders a raw `time` string ("14:30:00") as "2:30 PM"; wired into the two remaining raw string-interpolations of `scheduled_time` (`dashboard-page.tsx`, `docket/sections/events-section.tsx`). Native `<input type="date">`/`<input type="time">` controls (Docket Events, Case Law `decided_date`, Judgment `judgment_date`) were confirmed NOT part of the bug — they round-trip a plain `YYYY-MM-DD`/`HH:MM` string with no `Date` object involved; their on-screen widget appearance is governed by OS/browser locale, outside app-code control, and unaffected either way.

**C — Judgment↔Docket Matter linking.** Reused `docket_matter_judgments` (0029) and its existing read hooks (`useLinkedJudgments`, `useJudgmentDocketMatters`) unmodified — no migration. New `src/hooks/docket/use-docket-judgment-links.ts` adds the missing write side: `useCreateDocketJudgmentLink`/`useDeleteDocketJudgmentLink`, calling the exact live INSERT/DELETE policies (Docket access AND Judgment **ownership** — deliberately not mere discoverability, per 0029's own documented asymmetry with `docket_matter_case_law`, preserved exactly, not "fixed" to match Case Law's model). New `src/pages/docket/link-judgment-dialog.tsx` sources candidates from `useJudgments()` filtered client-side to `owner_id === current user` (server re-checks regardless). `docket/sections/judgments-section.tsx` gained a "Link Judgment" button, per-row Unlink action, and corrected empty-state copy (previously claimed linking happened "from the Judgment workspace," which was false on both sides — neither side had a linking control at all before this pass). The Judgment-side "Linked Docket Matters" card remains intentionally read-only, mirroring the Turn 2 Case-Law-initiated/Docket-read-only asymmetry, just mirrored in the other direction (Docket-initiated/Judgment-read-only) since RLS requires the _Judgment_ owner to act.

**D — Event Type/Stage controlled vocabularies.** `docket_events.event_type`/`.stage_at_event` remain unconstrained `text` — no CHECK constraint added; the live 0024 comment already documents this as deliberate, and adding one now risks rejecting real, already-stored free text. Instead, a frontend-only suggestion layer: `EVENT_TYPES`/`EVENT_STAGES` constants (`src/lib/validations/docket.ts`) and a new shared `ControlledVocabSelect` (`src/components/common/controlled-vocab-select.tsx`) — a canonical-option `<select>` plus an "Other (specify)" branch that reveals a free-text input, defaulting into "Other" mode for any pre-existing value not on the list so nothing is ever coerced or hidden. Event cards (`docket/sections/events-section.tsx`) now also show location (with icon) and Orders (previously only Outcome), both still conditionally rendered so a future/scheduled event's empty fields don't clutter the card.

**E — Bench Note autosave.** Investigation finding, not assumed: Bench Note content did NOT actually autosave before this pass — it required an explicit "Save content" click (`contentDirty` state + button), despite behaving like autosave superficially. (The Judgment content editor uses the identical manual-dirty-plus-button pattern and was left untouched — Part E scoped this to Bench Notes only, and Judgment's save flow is intertwined with finalize/lock logic reviewed carefully in Turn 1.) `bench-note-detail-page.tsx` now debounces (1200ms) real persistence via `useUpdateBenchNoteContent`, tracks actual mutation state (not just local dirty-tracking) through a new shared `SaveIndicator` component (`src/components/common/save-indicator.tsx`, states: saving/saved/error-with-retry), and flushes any pending debounce synchronously on unmount so navigating away mid-typing doesn't drop the last edit. No manual Save button remains for content.

**F — Bookmark navigation.** Quick Code bookmarks previously always opened the generic Quick Codes list (`() => ROUTES.quickCodes`, ignoring the id) since Quick Codes have no standalone detail route. Now the bookmark link carries `?qc=<id>`; `quick-codes-page.tsx` reads it, scrolls the matching row into view, and applies a 2.5s highlight flash, then clears the param (least-disruptive fix, no new route). Separately, a new `useBackNav` hook (`src/hooks/use-back-nav.ts`) lets a detail page's "Back" button honor `location.state.{backTo,backLabel}` if the caller supplied it, falling back to that page's own default list route otherwise — the Bookmarks page now passes `{backTo: ROUTES.bookmarks, backLabel: "Back to Bookmarks"}` on every entity type it links to (Docket Matter, Judgment, Case Law, Bench Note), so returning from a bookmarked item goes back to Bookmarks specifically, not always that entity's own list. Deliberately not a general browser-history stack — just one opt-in state convention, extensible later (e.g. from Search results) without rework.

**G — Legal taxonomy.** Investigation finding: this project already has TWO deliberately separate tagging architectures — (1) a global, admin-curated `tags` table (0006) joined via `case_tags`/`bench_note_tags`/`statute_tags`/`case_law_tags` (INSERT/DELETE admin-only on `case_law_tags`, confirmed via RLS, not merely the frontend's own read-only comment), and (2) `docket_matter_tags` (0026)/`judgment_tags` (0028), each migration explicitly stating it is "deliberately NOT connected to the existing global `tags` table" — free-text, owner-scoped, no admin bypass. Neither is duplicated or altered. New `src/lib/legal-taxonomy.ts` defines a curated, versioned, Commonwealth-Caribbean-Magistrates'-Court topic list (Evidence, Criminal Procedure, Narcotics, Road Traffic, Sentencing, Family and Children, Civil and Summary Jurisdiction — illustrative, extensible by editing the file, never a migration) consumed by a new shared `TagInput` (`src/components/common/tag-input.tsx`, native `<datalist>`-backed autocomplete, zero new dependency) wired into the two genuinely free-text systems only (Docket Matter tags, Judgment tags) — Case Law's admin-only `tags` join is untouched, correctly, since it already is a real backend-enforced canonical taxonomy needing no frontend suggestion layer.

**I — Module-scoped search.** Confirmed `docket-list-page.tsx`/`useDocketMatters(search)` already correctly calls the scoped `search_docket_matters` RPC (no change). Two real gaps found and fixed: the Judgments list had NO search box at all; the Case Law list had only an in-memory substring filter over `case_name`/`citation`/`court` (missing `summary`/`full_text` entirely, and not the real `search_case_law` RPC). A new shared `useScopedSearchIds` hook (`src/hooks/use-scoped-search.ts`) calls a given `search_X` RPC and returns just the matching id set; both list pages now intersect that set with their already-fetched, already-tab-categorized rows (Draft/Final/Discoverable; Canonical/Mine/Discoverable), preserving existing categorization/RLS-scoped field selection while gaining real full-text scope and quality. Global Search (`global_search()`) is untouched and remains explicitly cross-module. Tag-click-to-filter: investigated and found NOT to exist in any form today (Docket/Judgment tag Badges have only a remove "×" action, no click handler at all) — so the "clicking a tag triggers Global Search" premise was false, but so is "clicking a tag filters the workspace"; building that now, against the current freeform tag system, would likely be thrown away once/if a real tag-driven browse view is designed — recorded here as an explicit, reasoned DEFERRAL, not an oversight.

**J — Visual design.** Given no in-session ability to render/screenshot the app (text-only sandbox), a full page-by-page redesign could not be verified and was deliberately NOT attempted — an honest scope boundary, not a shortcut. What was done, centrally, in `src/index.css`/`tailwind.config.ts` (cascades app-wide, no per-page changes): light-mode `--background` warmed from stark white (`0 0% 100%`) to a subtle warm ivory/stone (`40 20% 98%`), with `--card` staying pure white so cards read as resting on paper; a new restrained `--brass`/`--brass-foreground` token pair (muted antique-gold, deliberately desaturated — "not meant to read as bright gold") plus a new `Badge` `"canonical"` variant, applied to the four existing "Canonical" badges (Case Law list/detail, Legislation detail, Docket's Case Law tab) in place of the generic `outline`/default treatment. Compiled and verified via the Tailwind CLI (same method used for the H2-heading fix in Turn 1) — `bg-brass/10`/`border-brass/30`/`text-brass-foreground` all generate correctly and the new `--background` value is present in the compiled CSS. Dark mode already reads as a genuine dark navy/charcoal (`--background: 222 47% 7%`) and was left as-is. Broader modernization (icons, empty states, skeletons, dialogs, transitions, spacing system) is DEFERRED — the existing components (Card's `rounded-xl border shadow-sm`, EmptyState, Skeleton) were judged already reasonably coherent on inspection, and risking a sweeping change with no way to visually confirm the result was assessed as worse than a small, confirmable improvement plus honest disclosure.

**K — Legislation workspace.** Investigation finding: `statutes` (0005) already exists — flat (no Part/Chapter/Section hierarchy), admin-write-only RLS, its own `search_statutes` RPC (0010) already included in `global_search()` (0047) — but had **zero frontend surface**: no route, no nav item, and `search-page.tsx` carried an explicit code comment that statute search results had "no working link." Implemented: a new "Legislation" nav item/route (`ROUTES.legislation`/`legislationDetail`), `src/hooks/legislation/use-legislation.ts` (`useStatutes`/`useStatute`, read-only, no new table), `legislation-list-page.tsx` (single institutional list, no tabs — unlike Case Law/Judgments there is no personal-content split for canonical legislation — wired to `search_statutes` via the same `useScopedSearchIds`), and `legislation-detail-page.tsx` (title/code/jurisdiction/effective date, source-URL link, summary, and `full_text` rendered as plain preformatted text — never AI-summarized or rewritten, matching "do not present AI summaries as statutory text"). `BookmarkToggle` (already generic across entity types) now has a real destination for `entityType="statute"`; `bookmarks-page.tsx` and `search-page.tsx`'s dead `statute` links were both pointed at the new detail route. **DEFERRED, NOT built this turn:** Act/Part/Chapter/Section/Subsection structural hierarchy, amendment/version/effective-date-range tracking, and any curation/ingestion UI — `statutes` remains exactly the flat table it was, and populating/editing a row is still an admin/database-level operation only. A hierarchical redesign is a substantial, separate schema project (new tables for Part/Section, likely a self-referencing or path-based structure, plus a real migration+RLS+rollback-pretest cycle) that was judged too large to attempt safely as a late addition to an already very large session.

**H/L–T — Ingestion platform, AI/classification, source connectors, duplicate detection, review/publication workflow.** Investigated first, per instruction: confirmed (again) that this project has **zero deployed Supabase Edge Functions** and no AI-provider secret/integration anywhere in the codebase or project config. Per the explicit instruction not to fake infrastructure that doesn't exist, and given that even a "scaffold-only" migration for a review-queue/ingestion-job schema would itself need its own careful RLS design and rollback-pretest cycle (new authority questions: who may submit a batch, who may publish canonical content, how does a Draft/Needs-Review/Published state machine interact with the existing canonical-vs-personal Case-Law/Legislation model) — none of Parts H, L, M, N, O, P, Q, R, S, T were implemented as code or schema this turn. This is recorded here as the explicit, reasoned architectural boundary this pass stopped at, not an oversight. Recommended design (for a future dedicated pass, not built): (1) a provider-agnostic classification interface (e.g. `ClassificationProvider { classify(text): Promise<{topic, confidence}[]> }`) so no single AI vendor is hard-coded into Case Law/Legislation; (2) a Draft/Needs-Review/Published state machine as new columns on `case_law`/`statutes` (or a lightweight `ingestion_jobs` table) with publication gated by the existing admin-only write authority already in place, never a new bypass; (3) duplicate detection as a pre-insert check against existing normalized-citation/title/jurisdiction fields already on both tables, surfaced as a warning, never an auto-merge; (4) provenance fields (source URL, retrieval date, document fingerprint) added to `case_law`/`statutes` only when a real ingestion pipeline is being built to consume them — adding unused provenance columns speculatively now was judged not worth the schema churn. Automatic tagging (Part H) should propose from the SAME `LEGAL_TAXONOMY`/`tags` table established in Part G above, never invent parallel synonyms, once a real provider exists.

**Verification this turn:** `npx tsc --noEmit` clean after every file group above (re-run repeatedly, zero errors throughout). `npx eslint` clean on every touched file (one pre-existing `react-refresh/only-export-components` warning on `badge.tsx`, unrelated to this turn's one-line variant addition — same file already exported `badgeVariants` alongside the component before this pass). `npx tailwindcss` CLI compile used to prove the new `--background`/`--brass` tokens and `bg-brass/10`/`border-brass/30`/`text-brass-foreground` utility classes actually generate (same verification technique as the Turn 1 H2-heading fix). Date-fix correctness proven via a standalone Node script exercising the exact `parseDateOnly`/`formatDate`/`getLocalDateOnly` logic under four `TZ` values (Guyana UTC-4, Kiritimati UTC+14, UTC, India UTC+5:30) — confirmed the old code reproduced the reported bug in Guyana specifically and the new code does not, in any of the four zones. `vite build` was not re-attempted this turn — this sandbox's pre-existing `@rollup/rollup-linux-arm64-gnu` optional-dependency gap (documented in every prior turn) is unrelated to any change made here and was not re-litigated.

**A (follow-up) — Date-entry control audit and DD/MM/YYYY hardening.** A follow-up pass, prompted by an explicit confirmed-column-types audit (`docket_events.scheduled_date`/`judgments.judgment_date`/`case_law.decided_date`/`statutes.effective_date`/legacy `cases.filed_date`/`closed_date` all real `date`; `docket_events.scheduled_time` real `time`; `*.started_at`/`*.ended_at`/`*.created_at`/`*.revoked_at`/`judgments.finalized_at` all real `timestamptz`) confirming the schema types were already correct and explicitly directing no new migration. Two things were hardened, both still frontend-only:

1. `src/lib/utils.ts` gained a strict, explicitly-labeled DATE/TIME/TIMESTAMPTZ contract block plus three new helpers used only by the date-entry control below: `toDDMMYYYY` (ISO -> "DD/MM/YYYY" for edit controls, distinct from `formatDate`'s prose "20 Aug 2026"), `parseDDMMYYYYToISO` (strict validation — rejects non-existent calendar dates like 30 February via the same local-`Date`-construction-then-round-trip-check technique as `parseDateOnly`, never a UTC-string parse), and `maskDDMMYYYY` (pure string masking for typed digit entry, no `Date` involved at all).
2. **Native `<input type="date">` was audited and replaced.** Root finding: a native date input's on-screen text format is governed entirely by the browser's own locale setting (`navigator.language`/OS region), not by any HTML attribute, CSS, or app code — a magistrate on an `en-US`-configured browser would see MM/DD/YYYY regardless of anything Magistrate Wizard does, silently reintroducing the same day/month confusion this whole date-handling effort exists to eliminate. Relabeling text next to the input would not have fixed this. A new `DateOnlyInput` (`src/components/common/date-only-input.tsx`) replaces it everywhere a `date` column is entered: a masked DD/MM/YYYY text field (typed digits auto-format, strict calendar validation, invalid entries flagged inline rather than silently rolled over) plus a small self-built calendar-grid popover (month nav, day-click) — no native picker, no external date-picker dependency (none installable in this sandbox regardless), and the calendar grid itself is built entirely from LOCAL `Date` construction (`new Date(year, month, day)` for grid-layout math only, never `new Date(dateOnlyString)`), so it carries zero UTC-parsing risk of its own. Emits/accepts the same plain "YYYY-MM-DD" string the native input did, so every existing `react-hook-form` `field.value`/`field.onChange` call site is a drop-in swap. Wired into all five actual date-entry points: `docket/sections/events-section.tsx` (`scheduled_date`), `judgments/judgment-detail-page.tsx` and `judgments/create-judgment-dialog.tsx` (`judgment_date`), `case-law/case-law-detail-page.tsx` and `case-law/create-case-law-dialog.tsx` (`decided_date`). `statutes.effective_date` and legacy `cases.filed_date`/`closed_date` have no frontend edit form at all (admin/DB-only and legacy-unbuilt respectively, both confirmed unchanged from earlier in this pass) — `effective_date`'s only frontend appearance is the already-correct `formatDate` read-only display on the Legislation detail/list pages.

Re-verification: the exact reported reproduction (select 20/08/2026 for a Docket Event) was re-proven end-to-end — mask "20082026" -> "20/08/2026" -> parse to "2026-08-20" -> round-trip back through both `toDDMMYYYY` ("20/08/2026") and `formatDate` ("20 Aug 2026") — under `TZ=America/Guyana` (UTC-4), `TZ=UTC`, `TZ=Pacific/Kiritimati` (UTC+14), and `TZ=Asia/Kolkata` (UTC+5:30); all four zones PASS identically, and the same shared code path covers Judgment date/Case Law decided date/Statute effective date since they all go through `formatDate`/`DateOnlyInput`. Invalid-date rejection separately verified (30 February rejected; 29 February accepted only in an actual leap year). A separate check confirmed a genuine `timestamptz` string is never matched by the date-only detection regex and continues to roll across real UTC-instant boundaries exactly as it should (correct instant behavior, not a bug). No `+1`/`-1` day arithmetic exists anywhere in any of this. Still no migration — 0052 remains the latest applied.

---

## 19. Turn 4 — Consolidated Audit/Completion Pass (Quick Code Categories, Bench Note↔Legislation, Bench Notes Search, Terminology)

**Two new migrations this turn — `0053` and `0054`, both additive-only, both rollback-pretested (`BEGIN`/exercise/`ROLLBACK` in a single transaction against the live database, including a disposable test statute row and a disposable `auth.users` row impersonated via `set_config('request.jwt.claims', …)` to prove the RLS/guard-trigger behavior under `authenticated` role before committing) before being applied for real.** Given the enormous scope of the governing 36-section audit prompt, this turn deliberately triaged toward a small number of real, bounded, low-risk product-completion items explicitly requested and achievable without new external infrastructure, plus a terminology/accessibility sweep — rather than attempting the full ingestion/AI/source-connector/review-queue/admin-curation scope, which remains DEFERRED exactly as characterized in §18 Part H/L–T (unchanged this turn; not re-investigated, no new AI provider or Edge Function exists).

**0053 — `quick_codes.category` (nullable `text`, additive).** A lightweight, optional grouping label for personal Quick Codes, deliberately kept as a UI convenience (free text, no CHECK constraint, no controlled-vocabulary table) rather than a new taxonomy — Quick Codes remain explicitly separate from the legal-subject-matter taxonomy (§18 Part G's `LEGAL_TAXONOMY`). No RLS change: `quick_codes`' existing owner-only RLS already governs every column, including this new one, so no policy touches `category` specifically. Frontend: `QUICK_CODE_CATEGORIES` (`src/lib/validations/quick-code.ts` — Adjournments/Bail/Sentencing/Evidence/Procedural Orders/Warrants/Maintenance/Juvenile/Administrative, illustrative and editable, never enforced at the DB level) consumed via the existing `ControlledVocabSelect` shared component (§18 Part D) — the same canonical-list-plus-"Other"-escape-hatch pattern already used for Event Type/Stage, reused unmodified. `quick-codes-page.tsx` gained: the category field in the create/edit dialog, a category column (Badge) in the list table, and a category filter `<select>` populated from the union of the curated list and whatever custom values are actually in use (so a magistrate's own free-text categories remain filterable, not just the nine illustrative ones).

**0054 — `bench_notes` gains a fourth associable parent type, `'statute'`.** Widens `bench_notes_entity_type_check` from `('docket_matter','judgment','case_law')` to add `'statute'`, and `CREATE OR REPLACE FUNCTION bench_notes_entity_guard()` (the `SECURITY DEFINER` `BEFORE INSERT OR UPDATE` trigger from 0038/0039 that procedurally validates `entity_id` against the correct parent table since no declarative FK can span four tables) gains one new `elsif new.entity_type = 'statute' then select exists(select 1 from public.statutes where id = new.entity_id) into v_exists;` branch, otherwise byte-identical to the live 0039 function body. **No RLS policy changed** — `bench_notes` RLS is strictly author-only (`author_id = auth.uid()`) on all four commands with no parent-access cascade and no admin bypass, a fact specifically re-confirmed by re-reading 0038 before writing 0054, which is what makes widening the referenceable parent-type set safe: Bench Note privacy has never depended on whether the author can currently see the referenced parent, so a magistrate can privately note an Act they have read access to (all authenticated users, per `statutes`' own admin-write/open-read RLS) exactly as they already could privately note a Judgment or Case Law entry. Frontend: `BENCH_NOTE_PARENT_TYPES` (`src/lib/validations/bench-note.ts`) gains `'statute'`; `use-bench-note-parent.ts` gains a resolution branch (title/code); `create-bench-note-dialog.tsx` gains a Legislation search-and-select branch (sourced from the existing `useStatutes()`, §18 Part K) and a new optional `defaultParent` prop so a caller can pre-fill and lock the "About" field — used by a new "New Bench Note" button on `legislation-detail-page.tsx`, the first caller of `CreateBenchNoteDialog` outside the Bench Notes list itself. `bench-notes-list-page.tsx`/`bench-note-detail-page.tsx` both gained the `statute -> "Legislation"` label/route mapping (`PARENT_ROUTE.statute = ROUTES.legislationDetail`).

**Bench Notes search, extended beyond title-only.** Investigation finding: `bench_notes.search_vector` (0004) was ALREADY `to_tsvector('english', title || ' ' || content_text)` — title AND content, not title-only — and `search_bench_notes(p_query, p_limit)` (0010) already existed as a working RPC over that vector. The actual gap was purely frontend: `bench-notes-list-page.tsx` never called it, instead doing an in-memory `title.includes(q)` substring filter only. Fixed by adding `"search_bench_notes"` to the existing shared `useScopedSearchIds` rpc-name union (§18 Part I) and wiring the list page to intersect real full-text matches (title + content) with its already-fetched rows, exactly the same established pattern already used for Judgments/Case Law/Legislation. Also added, genuinely new this turn (no prior partial version existed): a Draft/Published status filter and an associated-content-type filter (Docket Matter/Judgment/Case Law/Legislation, populated only from types actually present in the user's own notes). Terminology note: the governing prompt's illustrative language said "Draft/Final," but the actual, already-established `note_status` enum values are `draft`/`published` (visible in the existing detail-page Status `<select>`) — the filter uses the real stored values/labels, per the standing "do not alter stored enum values merely for capitalization" principle, rather than introducing a mismatched "Final" label the database doesn't have.

**Terminology sweep (Statutes → Legislation).** The visible product's nav/routes/module name has been "Legislation" since §18 Part K; two leftover "Statute(s)" user-facing strings were found and corrected for consistency: `search-page.tsx`'s `TYPE_LABELS.statute` (used as both the results-group heading and the per-row type Badge) and its scope-description sentence ("...Cases, and Statutes" -> "...Cases, and Legislation"), and the identical `TYPE_LABELS.statute` in `bookmarks-page.tsx`. Grepped for any other user-facing "Statute"/"Statutes" string app-wide after the fix — none remain; the only remaining occurrences are internal identifiers (`useStatute`/`useStatutes` hook names, the `statutes` table itself, code comments), which are correctly left alone. Legacy `cases`/`case_parties` were NOT touched or removed — per the explicit instruction not to delete legacy Cases merely because Case Law exists, and no dependency/purpose investigation beyond what §18 already recorded was conducted this turn (still an open, non-blocking question, not re-litigated).

**Accessibility spot-check.** A full accessibility audit was judged out of scope for this turn's budget; a targeted, grep-driven check specifically for icon-only (`size="icon"`) buttons missing an `aria-label` was run across the entire `src/` tree. Every existing icon-only button already had one except a single genuine gap: the per-row "Unlink" judgment button in `docket/sections/judgments-section.tsx` had only a mouse-hover `title="Unlink"` with no `aria-label`, invisible to a screen reader. Fixed with `aria-label={`Unlink judgment ${judgment.title}`}`. No other gaps found by this check; a broader responsive/contrast/keyboard-nav audit remains DEFERRED (this sandbox has no browser/screenshot capability, so any such work could be implemented but not visually verified, consistent with §18 Part J's disclosed limitation).

**Development-only Cloudflare `allowedHosts` setting — retained and now committed.** `vite.config.ts`'s `server.allowedHosts: [".trycloudflare.com"]` (added and left deliberately uncommitted in the immediately preceding turn, pending this turn's audit) is a dev-server-only host-check allowlist, inert in production builds (`vite build` never reads `server.*`), does not weaken any Supabase RLS/auth/storage policy, does not embed a secret, and is scoped to the Cloudflare Quick Tunnel wildcard specifically rather than `allowedHosts: true`. Retained and committed this turn on the judgment that it is a stable, safe, low-risk convenience needed for the user's ongoing external-testing workflow (giving a temporary tunnel URL to an outside reviewer) and that leaving it perpetually uncommitted would silently lose it on every fresh clone.

**Verification this turn:** both migrations rollback-pretested (see above) before being applied for real via `apply_migration`; `get_advisors(type: "security")` run immediately after — zero NEW findings attributable to either migration (the existing pre-0053 warning set, e.g. `bench_notes_entity_guard` being `anon`/`authenticated`-executable as a `SECURITY DEFINER` function, is an unchanged, pre-existing pattern shared by every trigger function in this schema, not something introduced here). `database.types.ts` regenerated via `generate_typescript_types` and hand-reconciled against the two schema changes (`quick_codes.category: string | null` added to Row/Insert/Update; the stale, misplaced `bench_notes` doc-comment — which had actually described `bookmarks`' entity_type values, not `bench_notes`' own — corrected to accurately document the real four-value set and the 0038/0054 guard-trigger design). `npx tsc --noEmit` clean. `npx eslint` clean except the same six pre-existing warnings already documented in §18 (`badge.tsx`/`button.tsx`/`form.tsx`/`theme-provider.tsx` — none of which were touched this turn). `vite build` not completed — this sandbox's pre-existing `@rollup/rollup-linux-arm64-gnu` optional-dependency gap (documented in every prior turn) reproduced identically and is unrelated to any change made here; `tsc -b`'s project-reference build step, which runs first, completed with zero errors before the unrelated rollup crash.

---

## 20. Turn 5 — Legal Library Ingestion Phase (Case Law + Legislation Structural Extraction, Review, Provenance)

**Framing.** This turn addressed the bottleneck explicitly identified at its start: Magistrate Wizard's legal content libraries (Case Law, Legislation) had correct architecture but essentially no ingestion path beyond direct-to-canonical admin INSERTs, and the Legislation library was empty with no way to seed it short of raw SQL. The governing instruction was explicit that this phase builds and proves the ingestion MACHINERY with small controlled examples — not a mass-import of real legal content. That boundary was respected: exactly one disposable, clearly-labelled, non-canonical Legislation fixture and one disposable Case Law fixture were created to prove the pipeline end-to-end, then both were deleted before this turn ended (see Controlled Test below) — nothing fake or placeholder remains in the canonical library.

**Migrations `0055`–`0057`, all additive, all applied.** (`0053`/`0054` were applied live in the immediately preceding turn but their local `.sql` files had never been written — a genuine process gap, fixed retroactively at the start of this turn by writing both files to exactly match what was already live, before any new migration was started.)

- **`0055_legal_library_ingestion.sql`** — the core schema. `legal_sources` (id, name, jurisdiction, base_url, `source_type` constrained to `case_law`/`legislation`/`mixed`, `connector_type` constrained to `manual`/`direct_document`/`html_document`/`index_page`/`structured_feed`/`custom_adapter`, `status` constrained to `proposed`/`testing`/`approved`/`disabled`/`failed`, `canonical_trusted` boolean, notes, `last_checked_at`/`last_successful_import_at`) — admin-only RLS on all four commands, a REGISTRY of intended sources per the spec's explicit instruction not to build every connector now, not an active crawler; adding a row here fetches nothing. `import_batches` (groups jobs for a bulk-import UX, not used by the single-item ingestion UI built this turn but present for future batch tooling) and `import_jobs` (one row per document/URL: `content_type` case_law/legislation, `source_id`, `source_url`, `uploaded_document_id`, `status` queued/fetching/extracting/structuring/needs_review/ready/published/failed, `target_case_law_id`/`target_statute_id` mutually exclusive via CHECK, `extracted_metadata` jsonb, `extracted_text`, `proposed_tags` text[], `duplicate_warning`, `error_summary`, `retry_count`) — both admin-only RLS. `case_law` and `statutes` each gained a `review_status` column (`draft`/`needs_review`/`ready`/`published`, default `'published'` — an additive default that changes nothing about any pre-existing row) plus provenance columns (`source_id`, `source_url` already existed on both, `original_filename`, `document_hash`, `retrieved_at`, `import_job_id`) and structural-extraction columns: `case_law` gained `neutral_citation`, `reported_citation`, `judges`, `parties`, `issues`, `principles`, `key_passages`, `disposition` (all nullable text — "do not invent missing metadata" honored by leaving every one of these blank unless deterministically extracted or curator-supplied); `statutes` gained `short_title`, `instrument_type`, `act_number`, `chapter_number`, `enactment_year`, `commencement_note`, `amendment_note`, `is_current_version` (boolean, default true), `supersedes_statute_id` (self-FK, minimal versioning room per the spec's explicit "don't overbuild a full versioning engine, but don't assume one-Act-forever either"). A new `statute_provisions` table implements the Part/Chapter/Section/Subsection/Paragraph/Schedule hierarchy: self-referencing `parent_provision_id` (nullable — tolerates missing levels by design, e.g. Act→Section direct with no Part), `level` CHECK-constrained to the six values, `number`/`heading`/`body_text` preserving source formatting exactly (never flattened/re-derived), `sort_order` for document-order preservation, a `GENERATED ALWAYS ... STORED` `search_vector`. `documents`/`storage.objects` RLS extended with a `statute` branch reusing the existing polymorphic architecture and private bucket unmodified (no new bucket, no public storage introduced). `can_view_case_law()`/`can_edit_case_law()` (the two centralizing helpers from `0044`, called by every dependent table per that migration's own documented design intent) were updated to gate on `review_status = 'published' OR is_admin()`, which automatically propagated the draft-invisibility rule to `documents`/`docket_matter_case_law`/`quick_code_case_law`/`case_law_annotations` with zero additional code changes — confirmed via a systematic `pg_policy` audit before editing, exactly the pattern `0044` was designed to enable. A new `can_view_statute()` mirrors this for `statutes`/`statute_provisions` (no `can_edit_statute()` — statutes write remains unconditionally admin-only, no per-row ownership distinction needed). `search_statutes()` (0010, `SECURITY INVOKER`) was extended to also UNION-match `statute_provisions` full text, deduplicated per-statute via `DISTINCT ON (id) ORDER BY id, rank DESC` with a provision-specific `ts_headline` — proven live this turn (see Controlled Test) to surface a parent Act when a search term appears only inside one provision's body text, not the Act's own title/summary.
- **`0056a`/`0056b`** — `bookmarks.entity_type` turned out to be a real Postgres ENUM (`bookmark_entity_type`), unlike `bench_notes.entity_type`'s plain CHECK-constrained text, which Postgres will not let you `ADD VALUE` and use in the same transaction — split accordingly: `0056a` is the bare `alter type ... add value 'statute_provision'` (applied directly, no meaningful rollback-test exists for a pure additive enum label); `0056b` (rollback-pretested normally afterward) widens `bench_notes_entity_type_check` to include `statute_provision`, adds the matching branch to `bench_notes_entity_guard()`, and adds the matching branch to `validate_bookmark_entity()` (which also picked up a `set search_path = public` fix for a pre-existing security-advisor warning on that function, a small bonus fix bundled into an already-planned `CREATE OR REPLACE`).
- **`0057_legal_library_fk_indexes.sql`** — thirteen `create index if not exists` statements covering every 0055 foreign key the Supabase performance advisor flagged as unindexed (INFO level only — `case_law.import_job_id`/`.source_id`, `statutes.import_job_id`/`.source_id`/`.supersedes_statute_id`, `import_jobs.created_by`/`.source_id`/`.target_case_law_id`/`.target_statute_id`/`.uploaded_document_id`, `import_batches.created_by`, `legal_sources.created_by`). Pure additive indexing, no RLS/data change; the one pre-existing unrelated unindexed FK (`shares.shares_item_id_fkey`) was left alone as out of scope.

**Deterministic extraction (`src/lib/legal-extraction.ts`), no AI anywhere.** SHA-256 hashing (Web Crypto, no dependency) of either a `File`'s raw bytes or normalized text, used as the primary duplicate-detection signal. Citation/date/section-heading pattern extraction via plain regex (`extractCaseLawMetadata`: neutral citation `[YYYY] COURT NNN`, reported citation `(YYYY) VOL REPORTER PAGE`, a "DD Month YYYY" date pattern, and a first-line-containing-" v "case-name guess — every field left `undefined`, never guessed, if no match is found). A line-based, deliberately non-exhaustive Part/Chapter/Section/Subsection/Paragraph/Schedule parser (`extractLegislationHierarchy`) that opens a new provision on a recognized heading line and appends all other lines to the most recently opened provision's body (falling back to a single top-level wrapper if a document has no recognizable headings at all, so text is never silently dropped) — explicitly documented as a useful first pass a curator edits, not a full statutory grammar. Keyword-based (substring match against the existing curated `LEGAL_TAXONOMY_TOPICS`, `src/lib/legal-taxonomy.ts`) tag proposal, explicitly NOT AI and labelled as such everywhere it surfaces in the UI. `findCaseLawDuplicates`/`findStatuteDuplicates` query helpers implementing the spec's strong-signal-first rule (hash or citation/chapter/Act-number exact match → `strength: "strong"`; normalized title/name fuzzy match only when no strong signal exists → `strength: "possible"`) — never auto-merges, always returns for the curator's judgment.

**Honesty boundary, stated directly in the ingestion UI, not glossed over:** this sandbox has no npm registry access (confirmed again this turn — a live `npm install` attempt returned `403 Forbidden`) and therefore no PDF/DOCX text-extraction library, and this project has zero deployed Supabase Edge Functions and no AI-provider secret anywhere (both re-confirmed, not merely assumed from the prior turn's finding). Consequently: automatic text reading works only for `.txt` uploads (`readFileAsText`); PDF/DOCX require the curator to paste text (e.g. copied from the in-app PDF viewer opened alongside the import form) — the filename is still recorded for provenance either way. URL/source-adapter fetching is NOT implemented — building an unauthenticated or naively-authenticated server-side fetch endpoint without a way to test it end-to-end (no verified Edge Function deployment/invocation path in this environment) was judged worse than an honest scaffold; `legal_sources` is a reviewable intent REGISTRY only, and every "Source" row's `connector_type` is metadata, not a running connector. OCR is not implemented and not claimed — no scanned/image-only-PDF detection was built this turn since no upload path routes a PDF through any automatic extraction at all yet (the gap is upstream of where OCR would apply). AI-assisted classification is explicitly unavailable/not configured — tag proposals are the keyword heuristic described above, never presented as AI output.

**"Draft-row-first" ingestion architecture.** Rather than staging extracted data in `import_jobs` and only materializing a real `case_law`/`statutes` row at publish time (which would require a `documents` UPDATE policy to re-parent an already-uploaded file from a holding area to its final record — no such policy exists, and `documents.file_path` is UNIQUE, so a second row can't simply be inserted pointing at the same object either), the real canonical row is created immediately with `review_status: 'draft'` (or `'needs_review'` once extraction has actually run against curator-supplied text), a source document can attach directly to that real row's own id via the existing `useDocuments`/`useUploadDocument` hooks completely unmodified, and "Publish" is a plain `review_status` flip — never a data-copying step. Authorization for the `owner_id IS NULL`/canonical case is enforced by the pre-existing `case_law_ownership_guard()` trigger (0035, unchanged), not merely a client-side role check.

**New hooks.** `src/hooks/case-law/use-case-law.ts` gained `useCaseLawReviewQueue` (admin-only, `review_status IN (draft, needs_review)`, joined to `import_jobs` via the `case_law.import_job_id` FK in one query for `duplicate_warning` — never an N+1 per-row lookup), `useCreateCanonicalCaseLaw`, `useUpdateCanonicalCaseLaw`, `useSetCaseLawReviewStatus`, `useRejectCanonicalCaseLaw` (hard delete of a rejected draft); `useCaseLawList` (the ordinary browse hook) now filters to `review_status === 'published'` client-side after already having `review_status` in its select, so an in-progress draft never appears in the ordinary Case Law workspace. `src/hooks/legislation/use-legislation.ts` gained the mirror set for `statutes` plus full CRUD over `statute_provisions` (`useStatuteProvisions(statuteId)`, `useCreateProvision`, `useUpdateProvision`, `useDeleteProvision`) scoped to one Act at a time (never loading the whole library client-side, per the explicit performance instruction). New `src/hooks/legal-library/use-legal-sources.ts` (admin CRUD over `legal_sources`) and `src/hooks/legal-library/use-import-jobs.ts` (`useImportJobs`/`useUpdateImportJob`/`useDeleteImportJob` plus the two orchestration hooks `useIngestCaseLaw`/`useIngestLegislation` that run the deterministic extraction, duplicate check, and draft-row-plus-import_jobs-row creation described above in one mutation).

**Admin Legal Library workspace (`/admin/legal-library`, admin-role-gated route, new sidebar item under the existing admin-only section — not added to the ordinary magistrate sidebar).** Three tabs: **Sources** (register/remove `legal_sources` rows, explicitly labelled as a registry, not a crawler trigger); **New Import** (content-type toggle, optional file upload with the `.txt`-auto-read/PDF-paste-fallback behavior surfaced as an inline toast explaining the limitation the moment a non-`.txt` file is chosen, required-field-gated submit calling `useIngestCaseLaw`/`useIngestLegislation`, navigates straight to the new draft's detail page on success); **Review Queue** (separate Case Law and Legislation sections, each card showing the duplicate warning inline when present, editable core fields with a Save-edits action that only appears once a field has actually changed, Reject-with-confirmation — hard deletes the draft and its provisions/import job via cascade — and Publish, which is a one-line `review_status` update relying entirely on the already-proven RLS/trigger authorization rather than any new bespoke check).

**Legislation structured reader rewrite (`legislation-detail-page.tsx`).** Now fetches `statute_provisions` alongside the Act and, when any exist, renders a two-column reader: a left/collapsible table of contents (a `Sheet` slide-over on narrow viewports via a "Contents" button, a sticky `overflow-y-auto` column on `lg:` and above — no layout break on phone, per the explicit cross-platform instruction) listing every provision in document order, indented by level, plus main-pane content for whichever provision is selected via the URL (`/legislation/:id/section/:provisionId`, a stable id-based anchor per the spec's explicit "never derive a URL from display text" instruction — added as a second route entry pointing at the same page component, `provisionId` read via `useParams` and looked up from the already-fetched provisions list). Per-provision contextual actions: Bookmark (now `entityType="statute_provision"`), Copy provision text, Copy citation (`"{Act title}, {Level} {Number}"`), and New Bench Note (pre-filled and locked to that specific provision). An Act with zero recorded provisions falls back to exactly the pre-0055 flat `full_text` rendering, so no existing/future manually-entered Act without structural extraction is left with a broken reader.

**Bench Notes/Bookmarks provision-level wiring, completed end-to-end (the database side — `statute_provision` as a valid entity type — was already done in 0056a/0056b; the frontend wiring was completed this turn).** `BENCH_NOTE_PARENT_TYPES` gained `statute_provision`; a new `BENCH_NOTE_PICKABLE_PARENT_TYPES` (excluding it) governs the general "About" dropdown shown from the plain Bench Notes list, since there is deliberately no library-wide provision search picker (a single Act can have hundreds of provisions; the library overall, thousands) — a provision-scoped Bench Note can only be created via the "New Bench Note" action on that provision's own reader view, which supplies a locked `defaultParent`. `useBenchNoteParent` was refactored to compute and return a fully-resolved `href` per entity type (rather than the page maintaining a separate static `PARENT_ROUTE` map), since resolving a provision's parent statute id for its two-segment route (`/legislation/:statuteId/section/:provisionId`) requires the same lookup that already fetches the provision's label. `useBookmarkLabels` gained a batched (`.in("id", ids)`, no N+1) `statute_provisions` lookup embedding the parent statute's title via Supabase's FK-embed syntax, returning a `parentId` field the Bookmarks page uses to build the same two-segment route inline (the one place a small per-row special case was accepted rather than forcing every entity type through an identical single-id `TYPE_ROUTE` map).

**Controlled end-to-end test — proved via direct, real (non-rollback) database operations impersonating a real admin and a real magistrate profile (`set_config('request.jwt.claims', ...)`), since this environment has no verified interactive browser session to drive the actual React UI; the hooks built this turn are thin wrappers over exactly these same Supabase calls, so this is a faithful test of the pipeline's data-layer behavior, disclosed honestly as NOT a browser click-through.** One Legislation fixture (`[TEST FIXTURE — DO NOT PUBLISH] Roadway Safety Act`, two provisions including a Section 2 "Dangerous driving" body) and one Case Law fixture (`[TEST FIXTURE — DO NOT PUBLISH] Test v. Fixture`, full text mentioning hearsay/admissions) were created as `review_status: 'needs_review'`/`owner_id: null` by the impersonated admin. Verified, in order: (1) the impersonated magistrate saw **zero** rows for both while in `needs_review` state (`select count(*)` = 0 against both `statutes` and `case_law`); (2) the admin flipped both to `published`; (3) the magistrate then saw exactly 1 row for each; (4) `search_statutes('dangerous driving')` — a phrase appearing ONLY inside the provision body, not the Act's title/summary — returned the Act, with a provision-specific `ts_headline`, proving the 0055 search extension's core claim; (5) `search_case_law('hearsay admissions')` returned the fixture judgment with a correct headline; (6) the magistrate successfully inserted a `bench_notes` row and a `bookmarks` row both against `entity_type: 'statute_provision'`, proving the 0056a/0056b guard/validator branches; (7) a duplicate-hash lookup (`select ... where document_hash = '...'`) against the fixture's own recorded hash returned it, proving the pattern `findCaseLawDuplicates` uses works against the real schema. All seven fixture/association rows (both statute provisions, the statute, the case_law row, both import_jobs rows, the bench_note, the bookmark) were then deleted for real — confirmed via a final `count(*) = 0` check across every touched table — so nothing fake remains in the canonical library, per the spec's explicit "do not pollute canonical production content with fake legal material" instruction. **What remains untested by this pass:** an actual browser session driving the built React forms/pages themselves (Admin Legal Library tabs, the Legislation reader's Sheet/TOC interaction, the Bookmark/Bench-Note buttons as rendered) — the SQL-level test above proves the underlying data operations are correct and RLS-safe, but not that the UI wiring calling them is defect-free; a document upload attached to a draft record end-to-end (Documents architecture itself was proven working in an earlier turn; the new `statute` `entity_type` branch added to it in 0055 was not separately exercised this turn); and any real external source (no URL ingestion exists to test).

**Deliberately deferred, not built this turn, consistent with the spec's own explicit instructions not to fake missing infrastructure:** URL ingestion (needs a safe, testable server-side fetch boundary — SSRF protections, redirect/size/timeout limits — that this environment cannot verify end-to-end); any actual source connector/crawler (the `legal_sources` table is a registry only); AI-assisted extraction/classification (no provider, no server-side AI infra; the interface boundary described in §18 remains the intended future seam — nothing new was built here since nothing changed about that unavailability); OCR (no scanned-PDF detection since no PDF ever reaches automatic text extraction yet); batch import UI (the `import_batches` table exists and is referenced by `import_jobs.batch_id`, but no UI creates or lists batches — every import this turn is single-item); Legislation amendment/version-history UI (the `supersedes_statute_id`/`is_current_version` columns exist for future use; nothing surfaces or sets them yet beyond the raw column default).

**Verification this turn:** `npx tsc --noEmit -p .` clean throughout. `npx eslint` clean on every file touched this turn (the six pre-existing warnings on untouched shadcn/theme-provider files, already documented in §18/§19, are unchanged). `npm run build` (`tsc -b && vite build`) — the `tsc -b` project-reference build step completed with **zero errors** (a stricter check than the plain `--noEmit` run, and it caught three genuine type errors during development — a missing-required-field mismatch on `useCreateCanonicalCaseLaw`, an un-castable `Json` assignment, and a free-text `source_type` where the schema actually has a three-value CHECK constraint on `legal_sources` — all three fixed before this report); `vite build`'s bundling step itself still cannot complete in this sandbox due to the same pre-existing `@rollup/rollup-linux-arm64-gnu` optional-dependency gap documented in every prior turn (re-confirmed this turn: a direct `npm install @rollup/rollup-linux-arm64-gnu` attempt returns `403 Forbidden`, consistent with the already-documented no-registry-access constraint) — an environment limitation, not a defect in the code the type-checker already validated. `get_advisors(type: "security")` re-run after all three new migrations — zero new findings beyond the same pre-existing, already-documented `SECURITY DEFINER` EXECUTE-grant WARN pattern shared by every trigger function in this schema and the account-level leaked-password-protection WARN (both unrelated to this turn). `get_advisors(type: "performance")` showed only INFO-level unindexed-FK findings on the new tables, resolved by `0057`, plus the same long-standing set of pre-existing `unused_index` INFOs on unrelated tables (expected on a low-traffic development database; not evidence of a design defect).

---

## 21. Turn 6 — Independent Source-Audit Repair + Operationalization Pass

**Framing.** An independent audit inspected the actual repository at the Turn 5 commit (`725045c`) and found concrete gaps between the Turn 5 report and the real implementation: the original PDF/DOCX file was never actually uploaded despite the documents architecture existing (only its filename string survived); `case_law.import_job_id`/`statutes.import_job_id` were never written back, silently breaking the Review Queue's own duplicate-warning join; Publish/Reject touched only the parent row and never reconciled `import_jobs.status`, letting the audit trail drift; the New Import browser workflow made independent sequential Supabase calls with no atomicity; Source selection existed as a registry but New Import always submitted `source_id: null`; and Case Law had no normalized Court/Jurisdiction relationships at all despite that being discussed before Turn 5. Every one of these was independently re-confirmed against the live repository and database before any fix was written (§0's "inspect before modifying" rule), not merely trusted from the audit's prose. This turn's governing instruction was explicit: repair the audit's findings and operationalize the existing architecture, without a ground-up redesign, in preparation for eventual large-scale ingestion of Guyanese and Commonwealth legal authorities.

**Migrations `0058`–`0060`, all additive, all rollback-pretested before being applied for real, none of them editing `0055`–`0057`.**

- **`0058_legal_library_operationalization.sql`** — six parts in one migration. (1) `can_view_case_law()`/`case_law`'s own SELECT policy: the `is_discoverable = true` branch was an unconditional OR, independent of `owner_id`/`review_status` — unexploitable in practice (no UI path ever sets `is_discoverable` on a canonical, `owner_id IS NULL` row, and `owner_id IS NULL` can never equal `auth.uid()`), but scoped to `owner_id IS NOT NULL AND is_discoverable = true` at the RLS/helper level anyway so a draft canonical row can never leak via that flag regardless of any future bug. (2) `documents_parent_cascade_delete()` had no `'statute'` branch — a genuine pre-existing gap (not introduced in Turn 5) that orphaned `documents` metadata whenever a Legislation row was deleted; added, plus the matching `documents_cascade_delete_statutes` trigger. (3) Three new normalized reference tables — `legal_regional_groups` (Guyana / Regional / Commonwealth Caribbean / Eastern Caribbean-OECS / Persuasive Commonwealth), `legal_jurisdictions` (14 seeded, each belonging to exactly one regional group), `legal_authority_courts` (20 seeded, `jurisdiction_id` AND `regional_group_id` both nullable so a supranational/apex court like the CCJ or the Judicial Committee of the Privy Council, or an Eastern Caribbean Court of Appeal/Supreme Court serving multiple OECS member jurisdictions, is never forced into a fake single-jurisdiction home; `aliases text[]` with a GIN index specifically so "CCJ"/"Caribbean Court Justice"/"Caribbean Court of Justice" resolve to one canonical row, never three) — all three shared-reference-data RLS (open SELECT to all authenticated, admin-only write, same pattern as `courts`/`magisterial_districts`). Deliberately named `legal_authority_courts`, **not** `courts` — the existing `courts` table (0002) is the physical Guyana Magistrates' Courts used for Docket/court-assignment authority, a completely different domain; conflating the two would have been a real modeling error, not a naming nicety. (4) `case_law.court_id`/`case_law.jurisdiction_id`/`statutes.jurisdiction_id` — nullable, additive FK columns; the existing free-text `court`/`jurisdiction` columns are explicitly preserved, never dropped, and documented via column comments as the permanent display fallback whenever the normalized relationship is unset. A deterministic (case-insensitive exact match only, courts also checked against `ANY(aliases)`) backfill ran once at migration time — proven against the one real pre-existing `case_law` row (`court: "vigilance"`) that `jurisdiction_id` correctly backfilled to Guyana while `court_id` correctly stayed NULL, since "Vigilance" matches no canonical court or alias — no mapping is ever invented. (5) Six transactional `SECURITY DEFINER` RPCs — `create_case_law_import`/`create_legislation_import` (draft row + `statute_provisions`, Legislation only + `import_jobs` row + the row's own `import_job_id` back-link, all in one PL/pgSQL function body — this single change fixes both the "not transactional" and "`import_job_id` never wired" findings at once), `publish_case_law_import`/`publish_legislation_import` (atomically sets `review_status = 'published'` on the parent AND `import_jobs.status = 'published'` on the linked job), `reject_case_law_import`/`reject_legislation_import` (atomically reconciles the linked `import_jobs` row to `status = 'failed'` with an `error_summary`, THEN deletes the parent row — cascades handle `statute_provisions`/`documents`-metadata cleanup automatically; an explicit code comment states the caller MUST remove the Storage object via the Storage API BEFORE calling this function, since SQL cannot touch Storage — an honest, disclosed boundary, not a false claim of cross-system atomicity). Every one of the six: explicit `if not is_admin() then raise exception` as its first statement (defense in depth — the pre-existing `case_law_ownership_guard` trigger, unmodified, independently re-validates on the INSERT itself regardless of the calling function's privilege escalation, since triggers read `auth.uid()` from the session GUC); `set search_path = public`; EXECUTE revoked from the schema-default PUBLIC grant and restricted to `authenticated` only (mirroring the 0043 precedent). (6) `search_case_law_scoped()` — a **new** function, not a `CREATE OR REPLACE` of the existing 2-argument `search_case_law()` (avoids any ambiguous-overload/grant-loss risk to that already-relied-upon function; confirmed via `information_schema.role_routine_grants` that it currently has PUBLIC/anon EXECUTE by schema default, matching the 0044 precedent) — supports an optional `p_query` (null/empty returns all matching rows at rank 0, for pure filter-browsing), `p_court_id`, `p_jurisdiction_id`, `p_tag_id`, `SECURITY INVOKER` so RLS applies automatically; plus `case_law_counts_by_court()`/`case_law_counts_by_jurisdiction()`, small `SECURITY INVOKER` count RPCs for Browse-UI badges.
- **`0059_review_status_state_sync.sql`** — completes the "audit all transitions, build a coherent state machine" repair: `set_case_law_review_status()`/`set_legislation_review_status()` handle the non-terminal `draft`/`needs_review`/`ready` transitions, syncing the linked `import_jobs.status` whenever that value exists in both vocabularies (`import_jobs.status` has no `'draft'` value, so a transition back to `draft` intentionally leaves the job's own status where it was rather than inventing a mapping); `'published'` and reject/delete are deliberately rejected by these two functions with an explicit error directing the caller to the 0058 publish/reject RPCs instead, so there are never two different code paths that can both set `'published'`.
- **`0060_legal_library_tag_apply.sql`** — `apply_case_law_tags()`/`apply_statute_tags()` repair the §15 finding that `proposeTags()` output was stored in `import_jobs.proposed_tags` but nothing ever turned a proposal into a real `case_law_tags`/`statute_tags` relationship (Turn 5's "Automatic tags: YES" was too broad — extraction proposes, nothing applies). Each function: admin-only; finds an existing `tags` row by case-insensitive exact name match first, only creates a new canonical `tags` row when none matches (the `tags` table itself has no case-insensitive uniqueness constraint, so this find-first behavior is what actually prevents synonym duplicates — "Hearsay"/"hearsay"/"Hearsay " never become three rows); reconciles the record's tag links to exactly the supplied name set in the same transaction (removes what's no longer wanted, adds what's new) — one call, not a client-side sequence of independent inserts/deletes.

All three migrations were rollback-pretested (full body + `DO $$ ... RAISE EXCEPTION ... END $$` assertion blocks + `set local role authenticated` / `request.jwt.claims` impersonation of the same real admin and real magistrate profiles used in Turn 5, wrapped in a single `execute_sql` call ending in `rollback`) before being applied for real via `apply_migration` — every assertion passed on the first attempt for all three.

**Frontend rewiring (`src/hooks/legal-library/use-import-jobs.ts`, `src/hooks/case-law/use-case-law.ts`, `src/hooks/legislation/use-legislation.ts`).** `useIngestCaseLaw`/`useIngestLegislation` now call `create_case_law_import`/`create_legislation_import` instead of independent `.insert()` calls, then upload the original file (if supplied) via a newly-extracted `uploadDocumentToEntity()` (factored out of `use-documents.ts`'s `useUploadDocument` so the ingestion flow can attach a document to an id that doesn't exist until the RPC returns it — the hook wrapper is unchanged, zero behavior change for every existing caller), then links the resulting `documents.id` onto `import_jobs.uploaded_document_id`. The document hash now prefers hashing the **original file's bytes** (`sha256File`, a function that existed in `legal-extraction.ts` since Turn 5 but was never actually called) when a file is attached, falling back to the pasted/typed text hash otherwise — Turn 5's implementation always hashed text only, so a PDF submitted with no pasted text got no duplicate-detection signal at all; that gap is closed. If the RPC succeeds but the Storage upload fails, the draft/job are NOT rolled back (they still exist, nothing is silently lost) and the failure is surfaced via a toast naming the retry path, rather than either silently swallowing it or discarding a otherwise-valid draft. `useSetCaseLawReviewStatus`/`useSetStatuteReviewStatus` route `'published'` through the 0058 publish RPCs and the other three statuses through the 0059 sync RPCs. `useRejectCanonicalCaseLaw`/`useRejectCanonicalStatute` now take `{ id, reason }`, remove any attached documents' Storage objects via the Storage API FIRST (throwing a descriptive "left in place, retry" error if that fails, rather than proceeding to delete the parent row and orphan the blob), THEN call the 0058 reject RPC.

**Source selection (§7/§8).** New Import gained a Source dropdown (`useLegalSources()`, grouped Approved / Not yet approved) wired to the real `source_id`, replacing the previous hardcoded `null`. An explicit UI note states the three-way distinction the audit required: Jurisdiction (the country the case is FROM), deciding Court (who decided it), and Source repository (which registry the document came from) are independent and must never be conflated — the same case can arrive from two different approved repositories while remaining the same deciding court.

**Court/Jurisdiction UI (`src/hooks/legal-library/use-legal-taxonomy.ts`, new).** Read-only hooks over the three 0058 reference tables (`useLegalRegionalGroups`/`useLegalJurisdictions`/`useLegalAuthorityCourts`) plus `useCaseLawCountsByCourt`/`useCaseLawCountsByJurisdiction`. New Import and the Review Queue cards both gained canonical Jurisdiction/Court `<select>`s (Case Law) or canonical Jurisdiction `<select>` (Legislation, which has no deciding-court concept) alongside the existing free-text fields, explicitly labelled "needs review" when unset — nothing is silently guessed.

**Case Law Browse by Court/Jurisdiction (`case-law-list-page.tsx`).** The Canonical tab gained Jurisdiction and Court filter `<select>`s (with result-count badges from the two new count RPCs) and a "Clear" action; once either filter (or the existing text search) is active, the Canonical tab's rows come from `search_case_law_scoped()` — a `SECURITY INVOKER` RPC, so results are automatically bounded by the caller's own RLS, never fetched unfiltered and trimmed in the browser. A Tag filter parameter exists on the RPC and the hook (`useCaseLawScopedSearch`) but has no `<select>` wired into this page yet, since the canonical `tags` table currently has zero seeded rows — noted honestly below rather than built against nothing to fill the checklist.

**Review Queue upgrade (`legal-library-admin-page.tsx`).** Both review cards gained: a Source name display plus a Source URL link where present; an `OriginalDocumentLink` component that resolves a signed view URL on demand (no signed URL is minted until clicked, and only for a document the admin's own RLS already permits reading) or an honest "no original file attached" note; canonical Court/Jurisdiction correction `<select>`s alongside the free-text fields; a `TagReviewEditor` — proposed tags (from `import_jobs.proposed_tags`, now actually selected in the review-queue query, which previously fetched only `duplicate_warning`) and already-applied tags shown as toggleable chips, a free-text add field, and a "Save tags" action that calls `apply_case_law_tags`/`apply_statute_tags` — nothing is applied until the reviewer explicitly saves; a content-type badge and the linked `import_jobs.status` badge. Duplicate-warning surfacing itself needed no separate UI fix — the existing embedded-select join (`case_law.import_job_id` → `import_jobs.duplicate_warning`) was already correct in Turn 5's code; it simply had nothing to join against because `import_job_id` was never written. With that fixed at the RPC layer, the existing query starts working without any query-shape change.

**Security model, unchanged in substance, re-stated for this turn's additions.** Every new RPC re-checks `is_admin()` explicitly even though it also runs `SECURITY DEFINER`, and the pre-existing `case_law_ownership_guard` trigger provides a second, independent enforcement layer that cannot be bypassed by any function's privilege escalation. The three new reference tables follow the exact shared-reference-data RLS shape already established by `courts`/`magisterial_districts`. No RLS was weakened anywhere this turn; the one RLS change (`is_discoverable` scoping) strictly narrows what an edge case could theoretically expose, never widens it. No admin bypass was introduced into private Docket/Judgment/Bench-Note/personal-Case-Law content — every new function and table concerns only the canonical Legal Library.

**Deliberately not built this turn, consistent with the governing instruction's explicit "do not fake it" boundary and its "do not begin mass-import" boundary:** real PDF/DOCX text extraction (still curator-pasted, now honestly labelled as such at the point of upload rather than only as a toast); OCR; safe server-side URL ingestion; any actual source connector/crawler; AI-assisted classification; a Tag filter control on the Browse UI (the RPC parameter and hook exist; no tags exist yet to filter by); a batch-import UI (`import_batches`/`import_jobs.batch_id` remain wired at the schema level only); scraping or bulk-populating any real court/jurisdiction's case law or legislation.

**Verification status at the point this report was written:** `npx tsc -b --force` clean across the entire project after every edit in this turn (checked repeatedly, not only once at the end). `npx eslint . --ext ts,tsx --report-unused-disable-directives` (the project's actual configured command, `--max-warnings 0` omitted from this particular invocation to see the raw count) reported **0 errors, 6 warnings, all six on files this turn never touched** (`badge.tsx`, `button.tsx`, `form.tsx`, `theme-provider.tsx` — the identical pre-existing set documented in Turn 5's own report) — meaning the project's actual configured `npm run lint` script (which does pass `--max-warnings 0`) would still exit non-zero on a totally clean pass of this turn's own work, because of those six pre-existing warnings, not anything this turn introduced; that distinction is reported explicitly rather than calling this "lint clean." `vite build`'s bundling step remains blocked by the same pre-existing `@rollup/rollup-linux-arm64-gnu` sandbox gap documented in every prior turn (re-confirmed, unrelated to this turn's code). The Supabase MCP tools (database queries, migration application, advisors) became unavailable partway through this turn due to a sustained platform-side outage (every call returned "claude-sonnet-5 is temporarily unavailable ... auto mode cannot determine the safety of this action") — migrations `0058`–`0060` were rollback-pretested and applied successfully BEFORE the outage began, so the schema/RPC layer carries real, passing rollback-battery verification, but the planned controlled-fixture end-to-end exercise of the newly rewired frontend hooks against a live database, and a fresh `get_advisors` run, could not be completed this session as a result. See the final report for the precise, itemized breakdown of what carries live-tested verification versus what carries rollback-pretest-only verification pending that outage clearing.

---

## 22. Turn 7 — Persistent Bulk Ingestion, Batch Accounting & Stabilization Pass

**Framing.** Turn 6 operationalized single-item ingestion but left `import_batches`/`import_jobs.batch_id` wired at the schema level only — no UI created or listed a batch, and Turn 6's own report named a batch-import UI as deliberately deferred. This turn (spanning two consecutive governing sessions, reported together since the second continued and completed the first's unfinished threads without any intervening architectural change) built that UI end-to-end, then subjected it to a live-user acceptance pass that surfaced a concrete defect — a persisted batch reporting "5 documents" in its list-row label while its own Batch Detail view showed only 1 — and used that defect as the entry point for a broader stabilization pass across the whole Legal Library ingestion surface. The governing instruction throughout was explicit that "duplicate is not failure," "every uploaded batch item should have an auditable outcome," and "Import Batch is the operational audit/history boundary" — this turn is the turn that made those three claims actually true against a live, persistent database rather than only against transient React state.

**Migrations `0061`–`0065`, all additive or narrowing (never widening) and applied live.**

- **`0061_case_law_publication_validation.sql`** — a live browser test found a canonical Case Law record could be published while `case_name` was still the system-default placeholder `"Untitled (pending review)"`, because the Publish button had no validation at all. Re-implements `publish_case_law_import`/`publish_legislation_import` (`CREATE OR REPLACE`, same signatures) to re-check the same placeholder-value list already enforced client-side (`src/lib/publication-validation.ts`) at the RPC boundary itself, so a record can never be published through any path — this admin UI, a future bulk tool, or a direct RPC call — while holding placeholder-only metadata. Case Law requires non-placeholder case name, citation, jurisdiction (with `jurisdiction_id` set), and court (with `court_id` set); Legislation requires non-placeholder title, code, and jurisdiction (with `jurisdiction_id` set). Decision date remains optional for both, per the governing instruction.
- **`0062_narrow_generic_court_alias.sql`** — a Turn 6 seed alias on one `legal_authority_courts` row was too generic and could false-positive-match unrelated text; narrowed to the specific alias actually intended. No structural change.
- **`0063_import_jobs_duplicate_status.sql`** — the root fix for "duplicate is not failure" (governing principle #3): `import_jobs.status`'s CHECK constraint gains a dedicated `'duplicate'` value, distinct from `'failed'`, and the bulk pipeline's exact-hash and citation-conflict branches (see below) persist that status instead of either fabricating a fake draft or recording nothing at all. Before this migration, a duplicate bulk item's only representation was an in-memory queue-item flag that vanished the moment the browser tab closed or navigated away — this is also the migration that made a duplicate outcome survive a page reload for the first time.
- **`0064_import_batches_expected_file_count.sql`** — the direct fix for the reported "5 documents / Batch Detail shows 1" defect. Adds one nullable `import_batches.expected_file_count integer`, recorded ONCE at batch creation from the curator's original file selection count (`files.length`, including files about to be rejected by client-side validation before any processing begins). Root cause, confirmed by live timestamp correlation between `import_batches.created_at` and the exact commit timestamp that shipped the persistent-bare-job-row architecture: the affected batches were created BEFORE that architecture existed, so their true per-file history was never persisted at all and is genuinely unreconstructable — not a residual bug in the code that created them after the fact. `expected_file_count` is deliberately never derived retroactively from `import_jobs` counts (exactly what made the legacy batches silently confusing); instead the accounting invariant going forward is **`count(import_jobs where batch_id = X) >= expected_file_count`**, checked directly against this stored baseline. A batch with `expected_file_count IS NULL` is honestly labelled "legacy — incomplete history" everywhere it's displayed, never silently treated as if it were complete.
- **`0065_case_law_citation_unique_idx_exclude_blank.sql`** — a genuine, previously-undetected defect found this turn while building a 13-item synthetic bulk-lifecycle test (not the user-reported batch-accounting bug — a separate, latent one). `case_law_citation_canonical_unique_idx` (`UNIQUE (citation) WHERE (owner_id IS NULL)`, Turn 5/0035) does not exclude the empty string, but both the single- and bulk-import pipelines legitimately create a canonical draft with `citation = ''` when nothing usable could be extracted yet (a blank citation is an expected, common `needs_review` state — publish-time validation, not draft creation, is where "Citation is missing" belongs, per 0061 above). Because the constraint didn't exclude blanks, only ONE canonical draft in the entire database could ever hold `citation = ''` at a time; a second, wholly unrelated blank-citation draft hit the raw constraint and surfaced as a misleading "already exists" error. Fixed by narrowing the partial index's `WHERE` clause to `owner_id IS NULL AND citation <> ''` — a pure relaxation (strictly fewer rows participate in uniqueness, never more), so it cannot invalidate any previously-valid state, and does not touch or weaken the independent 0061 publish-time gate.

**Persistent batch architecture (`src/hooks/legal-library/use-import-jobs.ts`, `src/hooks/legal-library/use-bulk-import.ts`).** Every bulk outcome — not just successful drafts — now produces exactly one persisted `import_jobs` row: `recordBulkNonDraftJob()` inserts a bare row (no `target_case_law_id`/`target_statute_id`) directly for a duplicate or genuine failure, under the same admin-only RLS policy that already permitted job creation; `recordBulkRejectedJobs()` does the same, batched, for files rejected by client-side validation before processing even began. The original filename and (for a duplicate) the existing record's id live in `extracted_metadata._originalFilename`/`_duplicateOfId` (read back via `readBareJobFilename()`/`readDuplicateOfId()`) since no `case_law`/`statutes` row exists for a bare outcome to hang that provenance off of. `useImportBatches()` (the persistent Import Batches list) and `useImportBatchDetail(batchId)` (a single batch's full row-by-row reconstruction) are both plain queries over these persisted rows — no client-side batch state exists anywhere; leaving the page, refreshing, or returning the next day all reconstruct the identical view from the database. Both hooks compute and expose `isLegacyIncomplete`/`isFullyAccounted` per the `expected_file_count` invariant above, surfaced identically in both the batch list and Batch Detail header via a shared `BatchCountSummary` component — a batch never again shows a document count with no explanation of whether that count is trustworthy.

**Duplicate vs. citation-conflict vs. failure, and "same case is not necessarily same file" (governing principles #3/#4).** The bulk per-file pipeline (`processOneItem` in `use-bulk-import.ts`) runs, in order: SHA-256 exact-file-hash check (`checkExactDuplicateByHash`, content-type-parameterized across `case_law`/`statutes`) — a true byte-identical duplicate, recorded as `status: 'duplicate'` with a `duplicateOfId` link the UI resolves into a "View existing" action straight to the existing canonical record; then, only for Case Law, a canonical-citation pre-check (`checkCanonicalCitationConflict`) — a DIFFERENT file whose extracted or filename-derived citation matches an already-canonical case (a different scan, report, or source copy of the same decision, not a duplicate file). Because `documents` is already a polymorphic table (`entity_type`/`entity_id`, no uniqueness constraint on the pair, confirmed by index inspection — no schema change needed), a citation conflict does not become a silent failure or a mysterious dead end: the new file is uploaded and attached to the EXISTING authority as an additional source document for curator review (`uploadDocumentToEntity("case_law", existingId, file)`, best-effort — the duplicate outcome itself is still recorded correctly even if this secondary attach fails), and the persisted `duplicate_warning` text says explicitly whether the attach succeeded. A genuine race (two items in the same bounded-concurrency batch sharing a citation, both passing the pre-check before either commits) still degrades safely: the raw `23505` from whichever insert loses the race is recognized by a `UNIQUE_VIOLATION_MESSAGES` map in `src/lib/utils.ts`'s `getErrorMessage()` and reclassified as a duplicate outcome, never left as a generic, misleading failure. Legislation has no equivalent pre-insert code/jurisdiction conflict check yet (see "Deliberately not built" below) — a `statutes_code_jurisdiction_idx` entry was added to the same friendly-message map as a defense-in-depth backstop, but the proactive attach-as-alternate-source UX described above is Case-Law-only for now.

**Review navigation and cache invalidation (governing principles #10/#11).** `ROUTES.adminLegalLibraryReviewCaseLaw`/`adminLegalLibraryReviewStatute` gained an optional `fromBatchId` argument threaded entirely through the URL (`&batch=<id>`, no new client-side global state — consistent with the page's pre-existing deep-link pattern), read back contextually as "which batch to open" on the Batches tab or "which batch to return to" on the Review tab; every "Review" action site (Batch Detail rows, the temporary bulk-queue list, the Batch Detail "Review next" button) now passes it through, and `CaseLawReviewCard` renders a "Back to batch" action alongside "View full record" whenever it's set. Publish/reject/status-change mutations were audited for stale-cache risk and found to already invalidate Batch Detail/Import Batches correctly via React Query's default query-key prefix matching (`use-case-law.ts`/`use-legislation.ts` each keep an intentionally-duplicated `importJobsQueryKey` constant rather than importing the shared one, specifically to avoid a circular import between those files and `use-import-jobs.ts` — confirmed via code read, not a bug, no fix needed).

**Retry (governing principle #17), scoped honestly to what a browser can actually support.** A genuinely useful cross-session retry is architecturally impossible for a job that never reached a successful `create_case_law_import` call (its `File` object cannot be reopened once the page/session ends — no browser API exists for that), so Retry operates ONLY on the in-memory queue from the same session: the large per-item worker was extracted from the bulk-run closure into a standalone `processOneItem(item, batchId, opts, counter)` so both the original bounded-concurrency run and a later single-item `retryItem(id)` call the identical pipeline, guaranteeing a retry can never silently create a duplicate Case Law row (the same hash/citation checks run again, exactly as a first attempt would). The Retry button appears only on a `status === "failed"` item in the temporary bulk queue and is deliberately absent from the persisted Batch Detail view, which has no live `File` to retry with — that asymmetry is intentional, not an oversight.

**Bulk Import UX (governing principle #18).** Once a batch is created, an "Open batch" button appears next to the temporary in-memory queue, handing the curator off to the persistent Import Batches record rather than leaving the transient upload panel as the only place that batch's status is visible. Batch labels are built from `formatDateTime()` (the project's existing unambiguous en-GB date/time formatter, never `toLocaleDateString()`) plus an optional Source-repository prefix, e.g. `"CCJ — Bulk import — 12 Aug 2026, 18:45 — 43 documents"`.

**Error messages (governing principle #38).** `getErrorMessage()`'s `UNIQUE_VIOLATION_MESSAGES` map now covers the Legislation code/jurisdiction collision (see 0065's neighbor fix above) as well as the pre-existing Case Law citation collision. Several call sites that previously interpolated a raw Supabase Storage or PDF-parser exception message directly into a user-facing string (document-rejection cleanup failures, "could not open original file," a PDF text-layer parser exception) now log the raw detail to the console and show a fixed, specific, safe explanation instead — consistent with the rest of the app's existing "never expose raw database/constraint internals" convention, not a new policy. The single-document import path also gained the same client-side file-validation pre-check (`validateFileForUpload`) bulk mode already had, closing a gap where an oversized/wrong-type single-document file previously reached the raw Storage API before failing.

**Global consistency findings (governing principle #23), all low-risk/high-value fixes, nothing manufactured.** The Sources tab rendered raw `status`/`source_type`/`connector_type` enum tokens directly (`proposed`, `case_law`, `direct_document`) instead of the human labels the New Import form already defined for the same values — three small label maps now cover both places. `StatuteReviewCard` had the identical raw-`job_status`-badge bug that `CaseLawReviewCard` had already been fixed for in an earlier pass — the same fix was never applied to its Legislation twin; now is. Two sibling review cards used different wording ("Save changes" vs. "Save edits") for the identical save action; unified to "Save changes."

**Bulk Legislation (governing principle #26) — investigated, deliberately not built this turn.** `bulk-ingestion-queue.ts` (hashing, bounded concurrency, file validation) is already content-type-agnostic; `useIngestLegislation`/`checkExactDuplicateByHash`/`useCreateImportBatch`/`recordBulkNonDraftJob` are already `content_type`-parameterized across `case_law`/`statutes`; `extractLegislationHierarchy` (Turn 5) is already a standalone, page-independent pure function callable from a bulk loop exactly the way `runPdfExtractionPipeline` is. No architectural blocker exists. What's missing is real, if modestly scoped, new work: a Legislation analogue of the citation-conflict pre-check, and — the one piece with no existing analogue at all — a heuristic for proposing an Act's code/title before a curator has typed anything, since Legislation's single-document path has always relied on the curator to type both. Given this environment's inability to behaviorally verify a brand-new multi-step upload UI (file picker, panel state, button wiring) without a browser, and given the number of other well-scoped, independently-verifiable fixes still pending this same turn, the decision was to defer rather than ship a UI-heavy new surface with only type-checking and database-level verification behind it — an honest scope decision per this section's own explicit permission to defer with reasons stated, not a discovered blocker.

**Synthetic bulk lifecycle test.** A disposable 13-file batch (5 clean text judgments, 1 low-confidence/placeholder-metadata item, 1 scanned/image-only requiring OCR, 2 exact-hash duplicates of one of the 5, 2 citation-conflict "different source, same case" items, 1 malformed-PDF failure, 1 zero-byte pre-processing rejection) was built directly against the live database (the same RPCs/inserts the real hooks call), proving: selected-file count exactly equals persisted-outcome count (13 = 13, `isFullyAccounted: true`); status totals exactly equal row totals (7 needs_review, 4 duplicate, 2 failed); a completely cold re-query (no client state at all) reconstructs every row with its correct display name, status, and reason — the strongest form of "leave and come back tomorrow" proof available without a live browser session. This exercise is what surfaced the 0065 blank-citation defect above. All fixture rows were deleted afterward; a `count(*) = 0` check across every touched table confirmed nothing remains.

**Regression coverage.** All nine existing pure-function/integration test suites in `scripts/tests/` (`test-bulk-ingestion.mjs`, `test-extraction-pipeline.mjs`, `test-extraction-quality.mjs`, `test-false-success-prevention.mjs`, `test-legal-extraction.mjs`, `test-page-awareness.mjs`, `test-persistent-batch-lifecycle.mjs`, `test-stale-state-and-publication.mjs`, `test-text-sanitize.mjs`) pass cleanly after every change this turn, run via `node --experimental-strip-types --import ./scripts/test-support/register.mjs`. Resource-limit constants (`MAX_FILE_SIZE_BYTES = 26214400`, the MIME allow-list) were re-confirmed to match the `documents` Storage bucket's own `file_size_limit`/`allowed_mime_types` exactly (`0011_storage.sql`) — the client-side check is not a stale guess at the server-side enforcement point. A full foreign-key/cascade audit across `import_batches`/`import_jobs`/`case_law`/`statutes`/`statute_provisions`/`documents` found no orphan risk: every operational nullable FK correctly `SET NULL` rather than cascading destructively (an `import_jobs` row survives its batch or its target draft being deleted — the audit trail is never silently erased by a later, unrelated delete), and `statute_provisions`/`statute_provisions.parent_provision_id` correctly `CASCADE` with their parent statute, which is the one relationship where cascading IS the correct behavior. Search/tag scoping re-confirmed unchanged and correct by code read: canonical list queries filter to `review_status = 'published'` client-side after already selecting it; the Review Queue query is separately scoped to `draft`/`needs_review` only; the two never overlap.

**Deliberately not built this turn, consistent with the same "do not fake it" boundary as every prior turn:** bulk Legislation import (investigated in depth, see above — not an architectural blocker, a scope/verification-risk deferral); a Legislation analogue of the Case-Law-only citation-conflict-attach UX; OCR (still not implemented — a "requires_ocr" item in this turn's synthetic test correctly landed as `needs_review` with no fabricated text, exactly as designed since Turn 5); any change to the underlying PDF-text-extraction engine.

**Security re-audit.** `get_advisors(type: "security")` re-run after all five new migrations shows only the same pre-existing WARN-level findings already documented in prior turns (`SECURITY DEFINER` functions executable by `anon` via PostgREST, present since Turn 5/6 and unrelated to this turn's changes) — no new finding introduced. `get_advisors(type: "performance")` shows only INFO-level `unused_index` entries (expected on a low-traffic development database) and the one pre-existing unrelated `shares.shares_item_id_fkey` unindexed-FK note, already known from Turn 5 and out of scope. No RLS policy was modified this turn; every new/changed function re-checks `is_admin()` explicitly per the established pattern; no service-role key or secret was introduced into any client file; Storage remains a private bucket accessed only via signed URLs minted on demand.

**Verification status.** `npx tsc --noEmit -p .` and `npx eslint` (project config, `--max-warnings 0`) both clean after every edit this turn, checked repeatedly rather than once at the end. All nine `scripts/tests/*.mjs` suites pass. Every live-database claim in this section was verified directly against the project's real Supabase instance via the MCP tools (impersonating a real admin profile through `set_config('request.jwt.claim.sub', ...)`for RPC-level checks, with the caveat — stated honestly rather than glossed over — that the same MCP connection also appears to run with `BYPASSRLS`, so a successful direct-table insert/select through it proves the underlying data/logic is correct but does NOT by itself prove RLS _policy_ enforcement; RLS policy claims in this section rest on reading the actual `CREATE POLICY` SQL, not on live insert behavior). No interactive browser session was available in this environment at any point in this turn; every UI-level claim (button placement, panel wiring, responsive layout) rests on code inspection and the project's own established Tailwind/component conventions, not on an actual rendered click-through — stated here, not implied otherwise.
