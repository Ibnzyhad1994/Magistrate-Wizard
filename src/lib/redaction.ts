...
export function normalizePixelRect(rect: { x: number; y: number; width: number; height: number }) {
...
export function normalizedToPixelRect(box: RedactionBox, page: { width: number; height: number }) {
  return {
    x: box.x * page.width,
    y: box.y * page.height,
