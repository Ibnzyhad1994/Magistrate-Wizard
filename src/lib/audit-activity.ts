  ...
  if (tableName === "statutes") {
    ...
    if (action === "insert") {
      return {
        category,
        badge,
        title: title ? `Legislation added: ${title}` : "Legislation added",
        subject: title,
      };
    }
    if (action === "delete") {
      return {
        category,
        badge,
        title: title ? `Legislation removed: ${title}` : "Legislation removed",
        subject: title,
      };
    }
    ...
    if (review && review !== asText(oldRow.review_status)) {
      return {
        category,
        badge,
        title: `Legislation ${review}${title ? `: ${title}` : ""}`,
        subject: title,
      };
    }
    return {
      category,
      badge,
      title: title ? `Legislation updated: ${title}` : "Legislation updated",
      subject: title,
    };
  }
  ...
  if (tableName === "case_law") {
    ...
    if (action === "insert") {
      return {
        category,
        badge,
        title: name ? `Case law added: ${name}` : "Case law added",
        subject: name,
      };
    }
    if (action === "delete") {
      return {
        category,
        badge,
        title: name ? `Case law removed: ${name}` : "Case law removed",
        subject: name,
      };
    }
    const review = asText(newRow.review_status);
    if (review && review !== asText(oldRow.review_status)) {
      return {
        category,
        badge,
        title: `Case law ${review}${name ? `: ${name}` : ""}`,
        subject: name,
      };
    }
    return {
      category,
      badge,
      title: name ? `Case law updated: ${name}` : "Case law updated",
      subject: name,
    };
  }
  ...
  if (tableName === "docket_matters") {
    ...
    if (action === "insert") {
      return {
        category,
        badge,
        title: subject ? `Docket matter created: ${subject}` : "Docket matter created",
        subject,
      };
    }
    if (action === "delete") {
      return {
        ...
        badge,
        title: subject
          ? `Docket matter permanently deleted: ${subject}`
          : "Docket matter permanently deleted",
        subject,
      };
    }
    if (!oldRow.deleted_at && newRow.deleted_at) {
      return {
        category,
        badge,
        title: subject ? `Moved to bin: ${subject}` : "Moved to bin",
        subject,
      };
    }
    if (oldRow.deleted_at && !newRow.deleted_at) {
      return {
        category,
        badge,
        title: subject ? `Restored from bin: ${subject}` : "Restored from bin",
        subject,
      };
    }
    ...
    return {
      category,
      badge,
      title: subject ? `Docket matter updated: ${subject}` : "Docket matter updated",
      subject,
    };
  }
...
export const changedFields = (oldData: Json | null, newData: Json | null): FieldChange[] => {
  const oldRow = asRecord(oldData);
  const newRow = asRecord(newData);
