...
export const INGEST_FILE_ACCEPT =
  ".pdf,.txt,.md,.markdown,.docx,.doc,image/png,image/jpeg,image/webp";
...
const isExecutableMagic = (bytes: Uint8Array): boolean =>
  (bytes[0] === 0x4d && bytes[1] === 0x5a) || (bytes[0] === 0x7f && asciiAt(bytes, 1, "ELF"));
...
const isDocxZip = async (bytes: Uint8Array): Promise<boolean> => {
  try {
    // Fetched on first .docx upload — JSZip has no business in the main bundle.
 ...
 */
export const assertFileContentMatchesKind = async (file: {
  name: string;
  type?: string | null;
  slice: File["slice"];
}): Promise<void> => {
