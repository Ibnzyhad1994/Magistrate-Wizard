...
/**
 * jsPDF is fetched on first burn rather than imported statically: it is
 * ~350 kB and only the redaction flow needs a writer. Node's jspdf build
 * exports a namespace; Vite's browser build exports the class.
 */
const loadJsPdf = (): Promise<JsPdfCtor> => {
  if (!jsPdfCtorPromise) {
    jsPdfCtorPromise = import("jspdf").then((mod) => {
      const ns = mod as unknown as { default?: unknown; jsPDF?: JsPdfCtor };
      const fromDefault =
        typeof ns.default === "function"
          ? (ns.default as JsPdfCtor)
          : (ns.default as { jsPDF?: JsPdfCtor } | undefined)?.jsPDF;
      const ctor = fromDefault ?? ns.jsPDF;
      if (typeof ctor !== "function") throw new Error("jspdf did not export a constructor.");
      return ctor;
    });
  }
...
type PdfjsLoaderGlobal = { __MW_PDFJS_LOADER__?: () => Promise<unknown> };
...
const loadPdfjs = async (): Promise<PdfjsModule> => {
  // Node needs pdf.js's legacy build. The test harness
  // (scripts/test-support/register.mjs) injects that loader instead of
  // this module importing it: naming the legacy entry here made Vite
  // bundle pdf.js twice (479 kB extra) for every browser and native build.
  const injected = (globalThis as PdfjsLoaderGlobal).__MW_PDFJS_LOADER__;
  if (injected) return (await injected()) as PdfjsModule;
...
export async function burnRedactedPdf(opts: {
  ...
  const [pdfjs, PdfCtor] = await Promise.all([loadPdfjs(), loadJsPdf()]);
