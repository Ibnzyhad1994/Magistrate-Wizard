import { isRememberExpired, rememberUntilFrom, REMEMBER_MS } from "@/lib/auth/session-policy";
...
export function readRememberPreference(local: SyncStorage): RememberPreference {
  ...
  try {
    ...
    return {
      ...
      rememberUntil: typeof parsed.rememberUntil === "number" ? parsed.rememberUntil : null,
    };
  } catch {
...
export function writeRememberPreference(local: SyncStorage, pref: RememberPreference): void {
...
export function setRememberMeFlag(
  ...
  writeRememberPreference(local, {
    rememberMe,
    rememberUntil: rememberMe ? rememberUntilFrom(now, options?.rememberMs ?? REMEMBER_MS) : null,
  });
}
