...
import type { DocketEvent, TablesInsert, TablesUpdate } from "@/types";
...
const mergeWithOutbox = (
  ...
  return merged.map((row) => {
    ...
    if (cached) {
      return {
        ...docketEventFromCached(cached),
        ...hearingFieldsFromEvent(row),
        id: row.id,
...
export function useCreateDocketEvent(matterId: string) {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      void pushAfterLocalSave(result.row, (saved) => syncDocketEventToGoogle(saved.id)).then(
        (push) => {
          if (!push.synced) {
...
export function useUpdateDocketEvent(matterId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, values }: { id: string; values: TablesUpdate<"docket_events"> }) => {
      try {
        const { data, error } = await supabase
          .from("docket_events")
          .update(values)
        ...
        const patch = {
          ...
          ...(values.outcome_at_event !== undefined
