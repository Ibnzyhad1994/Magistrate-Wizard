...
import {
  sanitizePreviewHtml,
  wrapDocxPagePreviewSrcDoc,
  wrapSanitizedPreviewSrcDoc,
...
export const DocumentViewerDialog = ({
  ...
  const loadPreview = async (source: Document, fileKind: typeof kind) => {
    ...
    if (fileKind === "docx") {
      ...
      if (derivative) {
        ...
        // Re-sanitise on every read. The derivative was sanitised when it
        // was generated, but the storage bucket is not a trust boundary;
        // the snapshot is `<style>…</style>` + body, so each half goes
        // back through its own sanitiser exactly as at generation time.
        const cached = await blob.text();
        const styleMatch = cached.match(/^\s*(?:<style\b[^>]*>[\s\S]*?<\/style>\s*)+/i);
        const safeStyle = styleMatch ? sanitizeDocxPageStyle(styleMatch[0], DOMPurify) : "";
        const safeBody = sanitizeDocxPageBody(
          styleMatch ? cached.slice(styleMatch[0].length) : cached,
          DOMPurify,
        );
        return {
          previewUrl: null,
          content: { mode: "docx-pages" as const, html: `${safeStyle}${safeBody}` },
        };
      }
      setPreparing(true);
      try {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        className="flex h-[85vh] w-full max-w-4xl flex-col gap-3 overflow-hidden sm:max-w-4xl"
