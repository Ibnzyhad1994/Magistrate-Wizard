...
import {
  ...
  useRevokeShare,
  useShares,
} from "@/hooks/shares/use-shares";
import { useAuth } from "@/hooks/use-auth";
import { formatDate, toTitleCase } from "@/lib/utils";
import { shareNoun, type ShareItemType } from "@/lib/shares";
...
export function SharingPanel({ itemType, itemId, canManage }: SharingPanelProps) {
  const { user } = useAuth();
  const noun = shareNoun(itemType);
  const { data, isPending, isError, error, refetch } = useShares(itemType, itemId);
  const revokeShare = useRevokeShare(itemType, itemId);
    ...
    <div className="mt-4 space-y-6">
      ...
      {isPending ? (
        ...
        <EmptyState
          ...
          description={`Active shares granted on this ${noun} will appear here.`}
          action={
            canManage ? (
              <Button size="sm" onClick={() => setCreateOpen(true)}>
                <Plus className="h-4 w-4" />
          ...
          <TableBody>
            {activeShares.map((share) => (
              <TableRow key={share.id}>
                ...
                <TableCell>{toTitleCase(share.permission)}</TableCell>
                <TableCell className="text-muted-foreground">
      ...
      <AlertDialog
        ...
        description={
          pendingRevoke
            ? `${pendingRevoke.recipient_display_name ?? "This recipient"} will immediately lose access to this ${noun}.`
            : undefined
        }
        confirmLabel="Revoke"
        ...
        <div className="space-y-4">
          ...
          {resolved && (
            <div className="space-y-1.5">
              ...
              >
                <option value="view">View (can see it, not edit it)</option>
